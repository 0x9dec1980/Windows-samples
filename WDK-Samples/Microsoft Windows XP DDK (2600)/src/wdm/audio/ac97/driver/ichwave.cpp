...
)
{
    ...
    *ServiceGroup_ = ServiceGroup;
    ...
    //
    PDMAOPERATIONS DmaOperations;

    //
    // Query for the new interface "DmaOperations" and use the function provided there
    // in case the interface is offered.
    //
    if (!NT_SUCCESS (Wave->DmaChannel->QueryInterface (IID_IDmaOperations, (PVOID *)&DmaOperations)))
    {
    ...
    {
        stBDList.pBDEntry = (tBDEntry *)DmaOperations->AllocateCommonBuffer (
                                    MAX_BDL_ENTRIES * sizeof (tBDEntry) * 2,
                                    &stBDList.PhysAddr,
                                    FALSE);
        DmaOperations->Release();
    }
...
NTSTATUS CMiniportWaveICHStream::GetNewMappings (void)
{
    ...
    else
    {
        ...
        if (Wave->AdapterCommon->ReadBMControlRegister16 (m_ulBDAddr + X_SR) &
            SR_CELV)
        {
            //
            // We starve.  :-(
