                     ...
                     PSP_DEVINFO_DATA DeviceInfoData)
{
    ...
    while (SetupDiEnumDeviceInfo (pspRequest->DeviceInfoSet, nIndex, DeviceInfoData))
    {
        ...
        //
        if (!lstrcmpi (szServiceName, TEXT("AC97SMPL")))
        {
            //
...
BOOL APIENTRY AddPropSheet (HPROPSHEETPAGE hPSP, LPARAM lParam)
{
    ...
    static HPROPSHEETPAGE   hPropSheetPage;     // handle of the property sheet.

    hPropSheetPage = hPSP;
    *pphPropSheet = &hPropSheetPage;
...
{
    // Load the library.
    *phDLL = LoadLibrary (L"ac97prop.dll");
    if (!*phDLL)
    {
