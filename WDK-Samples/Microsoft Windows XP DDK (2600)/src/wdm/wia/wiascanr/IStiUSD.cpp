...
STDMETHODIMP CWIAScannerDevice::GetCapabilities(PSTI_USD_CAPS pUsdCaps)
{
    ...
    if (!pUsdCaps) {
        WIAS_LERROR(m_pIWiaLog,WIALOG_NO_RESOURCE_ID,("CWIAScannerDevice::GetCapabilities, NULL parameter"));
        return E_INVALIDARG;
    }
    ...
    HKEY                hParametersKey)
{
    ...
    */
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = CreateInstance(&m_pScanAPI,FLATBED_SCANNER_MODE);
        if (m_pScanAPI) {
...
HRESULT CWIAScannerDevice::DoEventProcessing()
{
    ...
    while (bLooping) {
        ...
        if ( bRet || ( !bRet && ( ::GetLastError() == ERROR_IO_PENDING ))) {
            ...
            default:
                bLooping = FALSE;   // for loop to stop
            }
        } else {
