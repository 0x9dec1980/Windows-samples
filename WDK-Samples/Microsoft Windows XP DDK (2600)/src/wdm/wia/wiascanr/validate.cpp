    ...
    PMINIDRV_TRANSFER_CONTEXT pDataTransferContext)
{
    ...
    if (pDataTransferContext->tymed == TYMED_FILE &&
          (pDataTransferContext->guidFormatID != WiaImgFmt_BMP && pDataTransferContext->guidFormatID != WiaImgFmt_TIFF))
    {
        WIAS_LERROR(m_pIWiaLog,WIALOG_NO_RESOURCE_ID,("ValidateDataTransferContext, invalid format for TYMED_FILE"));
        return E_INVALIDARG;
    }

    if (pDataTransferContext->tymed == TYMED_MULTIPAGE_FILE && pDataTransferContext->guidFormatID != WiaImgFmt_TIFF)
    {
    ...
    if (pDataTransferContext->tymed == TYMED_CALLBACK && pDataTransferContext->guidFormatID != WiaImgFmt_MEMORYBMP) {
        WIAS_LERROR(m_pIWiaLog,WIALOG_NO_RESOURCE_ID,("ValidateDataTransferContext, invalid format for TYMED_CALLBACK"));
        return E_INVALIDARG;;
    }

    if (pDataTransferContext->tymed == TYMED_MULTIPAGE_CALLBACK && pDataTransferContext->guidFormatID != WiaImgFmt_TIFF) {
...
* History:
*
*    11/07/2000 Original Version
*
\**************************************************************************/
HRESULT CWIAScannerDevice::UpdateValidPages(BYTE *pWiasContext,
                                            WIA_PROPERTY_CONTEXT *pContext)
{
    HRESULT hr = S_OK;
    BYTE *pRootItemCtx   = NULL;
