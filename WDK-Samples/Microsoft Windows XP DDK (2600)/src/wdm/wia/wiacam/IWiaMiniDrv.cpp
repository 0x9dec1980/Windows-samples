...
HRESULT CWiaCameraDevice::AddObject(MCAM_ITEM_INFO *pItemInfo)
{
    ...
    //
    ConstructFullName(pItemInfo, wszTemp);
...
VOID CWiaCameraDevice::ConstructFullName(MCAM_ITEM_INFO *pItemInfo, WCHAR *pwszFullName)
{
    ...
    if (pItemInfo->pParent) {
        ConstructFullName(pItemInfo->pParent, pwszFullName);
...
HRESULT CWiaCameraDevice::BuildCapabilities()
{
    ...
    //
    hr = wiauGetResourceString(g_hInst, IDS_EVENT_DEVICE_CONNECTED_NAME, &m_pCapabilities[0].wszName);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");

    hr = wiauGetResourceString(g_hInst, IDS_EVENT_DEVICE_CONNECTED_DESC, &m_pCapabilities[0].wszDescription);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");
    ...
    //
    hr = wiauGetResourceString(g_hInst, IDS_EVENT_DEVICE_DISCONNECTED_NAME, &m_pCapabilities[1].wszName);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");

    hr = wiauGetResourceString(g_hInst, IDS_EVENT_DEVICE_DISCONNECTED_DESC, &m_pCapabilities[1].wszDescription);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");
    ...
    // WIA_CMD_SYNCHRONIZE
    //
    hr = wiauGetResourceString(g_hInst, IDS_CMD_SYNCRONIZE_NAME, &m_pCapabilities[2].wszName);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");

    hr = wiauGetResourceString(g_hInst, IDS_CMD_SYNCRONIZE_DESC, &m_pCapabilities[2].wszDescription);
    REQUIRE_SUCCESS(hr, "BuildCapabilities", "wiauGetResourceString failed");
...
HRESULT CWiaCameraDevice::DeleteCapabilitiesArrayContents()
{
    ...
    if (m_pCapabilities) {
        for (LONG i = 0; i < m_lNumCapabilities; i++) {
            SysFreeString(m_pCapabilities[i].wszName);
            SysFreeString(m_pCapabilities[i].wszDescription);
        }
