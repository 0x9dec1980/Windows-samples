...
HRESULT WiaMCamFreeThumbnail(MCAM_DEVICE_INFO *pDeviceInfo, BYTE *pThumb)
{
    DBG_FN("WiaMCamFreeThumbnail");
    
    HRESULT hr = S_OK;

    REQUIRE_ARGS(!pDeviceInfo || !pThumb, hr, "WiaMCamFreeThumbnail");
...
HRESULT FakeCamOpen(PTSTR ptszPortName, MCAM_DEVICE_INFO *pDeviceInfo)
{
    ...
    pPrivateDeviceInfo = (UNALIGNED  FAKECAM_DEVICE_INFO *) pDeviceInfo->pPrivateStorage;
    ptszRootPath = pPrivateDeviceInfo->tszRootPath;
    uiRootPathSize = sizeof(pPrivateDeviceInfo->tszRootPath) / sizeof(*ptszRootPath);
    ...
    if (_tcsstr(ptszPortName, TEXT("COM")) ||
        ...
        0 == lstrcmpi(ptszPortName, TEXT("AUTO")))
    {
        uiSize = GetSystemDirectory(ptszRootPath, uiRootPathSize);
        REQUIRE_FILEIO(uiSize, hr, "WiaMCamOpen", "GetSystemDirectory failed");
        if (uiSize > uiRootPathSize) {
            wiauDbgError("WiaMCamOpen", "Root path is too long, %d", uiSize);
            hr = E_FAIL;
            goto Cleanup;
        }

        lstrcat(ptszRootPath, TEXT("\\image"));
    }
    else
        lstrcpy(ptszRootPath, ptszPortName);
...
HRESULT SearchDir(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, PTSTR ptszPath)
{
    ...
    //
    lstrcpy(tszTempStr, ptszPath);
    lstrcat(tszTempStr, TEXT("\\*"));
    ...
    while (hr == S_OK)
    {
        ...
        {
            MakeFullName(tszFullName, ptszPath, FindData.cFileName);
...
HRESULT SearchForAttachments(FAKECAM_DEVICE_INFO *pPrivateDeviceInfo, MCAM_ITEM_INFO *pParent, PTSTR ptszMainItem)
{
    ...
    //
    lstrcpy(tszTempStr, ptszMainItem);
    ptcDot = _tcschr(tszTempStr, TEXT('.'));
    ...
    while (hr == S_OK)
    {
            ...
            !(_tcsstr(ptszMainItem, FindData.cFileName)))
        {
            ...
            //
            lstrcpy(tszFullName, ptszMainItem);
            ptcSlash = _tcsrchr(tszFullName, TEXT('\\'));
            if (ptcSlash)
                *(ptcSlash+1) = TEXT('\0');
            lstrcat(tszFullName, FindData.cFileName);
                     ...
                     WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppFolder, PTSTR ptszFullName)
{
    ...
    *ppFolder = NULL;
    ...
    //
    lstrcpy(tszTempStr, pFindData->cFileName);
    ptcDot = _tcschr(tszTempStr, TEXT('.'));
    if (ptcDot)
        *ptcDot = TEXT('\0');
                    ...
                    WIN32_FIND_DATA *pFindData, MCAM_ITEM_INFO **ppImage, PTSTR ptszFullName)
{
    ...
    *ppImage = NULL;
    ...
    //
    lstrcpy(tszTempStr, pFindData->cFileName);
    ptszDot = _tcschr(tszTempStr, TEXT('.'));
    if (ptszDot)
        *ptszDot = TEXT('\0');
...
{
    DWORD num = *((UNALIGNED DWORD *) pBuf);
    DWORD den = *((UNALIGNED DWORD *) pBuf + 1);
