...
Continue_It:
    ...
    if ((rop4 >> 8) == (rop4 & 0xff))
    {
        ...
        if (psoDst->iType != STYPE_BITMAP)
        {
            ...
            else if (ppdev->flCaps & (CAPS_NEW_MMIO | CAPS_BAD_PACKED_EXPANDS))
            {
                // We'll put some ROP3 code here for those chips that can
                // handle it.
                //
                // We don't want to execute the below code which handles
                // and ROP3 on any S3 chip when the colours are black and
                // white, because the first versions (at least) of the 868
                // and 968 have a bug when foreground is set to transparent
                // and background is set to overpaint.  This bug is evident
                // only at 8bpp, on some buttons.
            }
        ...
        else
        {
            #if defined(i386)
            {
             ...
             ((((rop4 >> 2) ^ (rop4)) & 0x33) == 0))
    {
        // GDI will never actually ask DrvBitBlt to do a true rop4 without a
        // mask bitmap.  But since we convert DrvPaint to a DrvBitBlt call,
        // we may be asking ourselves to handle this case.  It will occur
        // when painting complex regions with a transparent monochrome
        // brush.
