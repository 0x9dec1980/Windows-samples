...
BANK_MODE    bankm)
{
    ...
    if ((bankm == BANK_ON) || (bankm == BANK_ON_NO_WAIT))
    {
        ...
        {
            do {;} while (INPW(pbd->ulGp_stat_cmd) & 0x0200);
        }
...
PDEV*   ppdev)
{
    ...
    {
        // Only the 866/868/968 and newer chips can handle us writing
        // to the frame buffer at the same time that the accelerator
        // is doing an operation, so they're the only ones on which
        // we can use BANK_ON_NO_WAIT.

        ppdev->bankmOnOverlapped = BANK_ON_NO_WAIT;
        ppdev->pfnBankMap        = vNmBankMap;
        ppdev->pfnBankSelectMode = vNmBankSelectMode;
        pfnBankInitialize        = vNmBankInitialize;
    }
