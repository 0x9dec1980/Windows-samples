...
{                                                                   \
    ULONG ReturnedDataLength;                                       \
                                                                    \
    if (!DeviceIoControl((pmdev)->hDriver,                          \
...
LONG    cyScreen)
{
    ...
    apmb = LocalAlloc(LMEM_ZEROINIT, cBoards * sizeof(MULTI_BOARD));
    if (apmb == NULL)
    {
        DISPDBG((0, "bInitializeGeometry -- Failed LocalAlloc"));
        goto ReturnFailure0;
    }
...
HANDLE      hDriver)        // Kernel driver handle
{
    ...
    if (pmdev == NULL)
    {
        DISPDBG((0, "MulEnablePDEV -- Failed LocalAlloc"));
        goto ReturnFailure0;
    }
...
VOID MulDisablePDEV(DHPDEV dhpdev)
{
    ...
    LocalFree(pmdev->pmb);      // Undo 'bInitializeGeometry' allocation
