    ...
    if (psoSrc == NULL)
    {
        ...
        if (pdsurfDst->dt == DT_SCREEN)
        {
            ...
            Fill_It:
                ...
                pfnFill = vFillSolid;   // Default to solid fill
            ...
            if ((rop3 >> 4) == (rop3 & 0xf))
            {
                ...
                else // psoSrc->dhsurf != NULL
                {
                    if ((pxlo == NULL) || (pxlo->flXlate & XO_TRIVIAL))
                    {
                        ...
                        if (jClip == DC_TRIVIAL)
                        {
                            vCopyBlt(ppdev, 1, prclDst, rop4, pptlSrc, prclDst);
                            goto All_Done;
                        }
                        else if (jClip == DC_RECT)
                        {
                            if (bIntersect(prclDst, &pco->rclBounds, &rcl))
                            {
                                vCopyBlt(ppdev, 1, &rcl, rop4, pptlSrc, prclDst);
                            }
                            goto All_Done;
                        }
                        else
                        {
                            ...
                            do {
                                ...
                                if (c != 0)
                                {
                                    vCopyBlt(ppdev, c, ce.arcl, rop4, pptlSrc,
                                             prclDst);
                                }
        ...
        if (psoDst->dhsurf != NULL)
        {
            ...
            if (psoSrc->dhsurf != NULL)
            {
                ...
                if (pdsurfDst->dt == DT_SCREEN)
                {
                    ...
                    Screen_To_Screen:
                        ...
                        vCopyBlt(ppdev, 1, prclDst, 0xCCCC, &ptl, prclDst);
                        return(TRUE);
                    }
            ...
            else if (psoSrc->iBitmapFormat == ppdev->iBitmapFormat)
            {
                ...
                DIB_To_Screen:
                    ...
                    vXferNative(ppdev, 1, prclDst, 0xCCCC, psoSrc, pptlSrc,
                                prclDst, NULL);
                    return(TRUE);
                }
            }
