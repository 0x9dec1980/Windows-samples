...
RECTL*          prclClip)       // NULL if no clipping
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                vPatRealize8bpp(ppdev, prb);
            }

            pjBase  = ppdev->pjBase;
            pbe     = prb->apbe[IBOARD(ppdev)];

            ptd->pfnTrap  = vSoftwareTrapezoid;
...
SetUpForFilling:
    ...
    vTrapezoidSetup(ppdev, rop4, iSolidColor, prb, pptlBrush, &td, yTrapezoid,
                    prclClip);
