...
*
* This routine caches small circles in off-screen memory.
*
* Note that this routine is purely optional for implementation, and
* probably isn't even recommended.  It exists here primarily to
* illustrate how GCAPS_BEZIERS may be used.
*
\**************************************************************************/
...
LINEATTRS*  pla)            // Used for strokes only
{
    ...
    if (!(ppdev->flStatus & STAT_CIRCLE_CACHE))
        return(FALSE);
...
Draw_It:
            ...
            ptlSrc.x = pce->x - ppdev->xOffset;
            ptlSrc.y = pce->y - ppdev->yOffset;
            ...
            {
                vCopyBlt(ppdev, 1, &rclDst, 0xaacc, &ptlSrc, &rclDst);
            }
            else if (pco->iDComplexity == DC_RECT)
            {
                if (bIntersect(&rclDst, &pco->rclBounds, &rclTmp))
                    vCopyBlt(ppdev, 1, &rclTmp, 0xaacc, &ptlSrc, &rclDst);
            }
            else
            {
                ...
                do {
                    ...
                    if (c != 0)
                        vCopyBlt(ppdev, c, ce.arcl, 0xaacc, &ptlSrc, &rclDst);
    ...
    lDelta  = CIRCLE_DIMENSION * ppdev->cjPelSize;

    // Adjust 'pvScan0' so that GDI will start drawing in the first dword
    // of our temporary buffer.  GDI requires that 'pvScan0' and 'lDelta'
    // must both be dword aligned:

    pso->lDelta  = lDelta;
    pso->pvScan0 = (BYTE*) pvTmp
                 - (yCircle)      * lDelta
                 - (xCircle & ~3) * ppdev->cjPelSize;

    ASSERTDD((((ULONG) pso->pvScan0) & 0x3) == 0,
             "Surface must have dword alignment");
    ASSERTDD((((ULONG) pso->lDelta) & 3) == 0,
            "CIRCLE_DIMENSION should be dword aligned");
    ASSERTDD((CIRCLE_DIMENSION + 4) * CIRCLE_DIMENSION * ppdev->cjPelSize
            <= TMP_BUFFER_SIZE,
            "Shouldn't allow circles bigger than temporary buffer");

    // Erase old temporary buffer to blackness:

    memset(pvTmp, 0, lDelta * cy);

    // Get GDI to draw the circle into our temporary buffer:

    co.iDComplexity = DC_TRIVIAL;
    bo.iSolidColor  = 0xffffffff;
    ...
    // Now blt the result to our off-screen buffer, converting the
    // destination rectangle to relative coordinates:

    ptlSrc.x = xCircle;
    ptlSrc.y = yCircle;

    rclDst.left   = pce->x - ppdev->xOffset;
    rclDst.top    = pce->y - ppdev->yOffset;
    rclDst.right  = rclDst.left + cx;
    rclDst.bottom = rclDst.top + cy;

    vXferNative(ppdev, 1, &rclDst, 0xcccc, pso, &ptlSrc, &rclDst, NULL);
...
// MGA specific initialization:
    ...
    else
    {
        ...
        ppdev->ulLineControl = (blockm_OFF + pattern_OFF + bltmod_BFCOL +
                                ...
                                opcode_AUTOLINE_OPEN + (ulHwMix << 16));
    }
