...
// Do fast stretches only on x86, for now.  Alpha has the funky sparse
// space macros for accessing the frame buffer that would negate most
// of our speed wins, and the driver isn't yet supported on PPC or Mips.
    ...
    if (StrBlt.YDstCount > 0)
    {
        ...
        else
        {
            ASSERTDD(ppdev->iBitmapFormat == BMF_16BPP,
                     "Only handle stretchs at 8 and 16bpp");
