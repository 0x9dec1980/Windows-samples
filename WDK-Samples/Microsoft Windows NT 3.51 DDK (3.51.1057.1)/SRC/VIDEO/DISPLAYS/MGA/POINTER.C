...
FLONG       fl)
{
    ...
    // Figure out the number of bytes per scanline of the masks:
    //   8 bytes (64 pels) if we are using Bt485 or Viewpoint
    //   4 bytes (32 pels) if we are using Bt482
    ...
    memset(AndMask, 0xff, 512);
    memset(XorMask, 0x00, 512);

    // Get the bitmap dimensions.

    cxMask = psoMsk->sizlBitmap.cx;
    cyMask = psoMsk->sizlBitmap.cy;
    ...
    for (i = 0; i < cyAND; i++)
    {
        ...
        for (j = 0; j < cxAND; j++)
        {
            AndMask[i*ucByteWidth + j] = pjAND[j];
        }
    ...
    for (i = 0; i < cyXOR; i++)
    {
        ...
        for (j = 0; j < cxXOR; j++)
        {
            XorMask[i*ucByteWidth + j] = pjXOR[j];
        }
        ...
        (ppdev->RamDacFlags == RAMDAC_PX2085))
    {
        ...
        for (i = 0 ; i < 512 ; i++)
        {
            CP_WRITE_REGISTER_BYTE(pjBase + BT485_CURSOR_RAM_DATA, pjAND[i]);
        }
        ...
        for (i = 0 ; i < 512 ; i++)
        {
            CP_WRITE_REGISTER_BYTE(pjBase + BT485_CURSOR_RAM_DATA, pjXOR[i]);
        }
    }
    else    // Download to Bt482
    {
        ...
        for (i = 0 ; i < 128 ; i++)
        {
            CP_WRITE_REGISTER_BYTE(pjBase + BT482_OVRLAY_REGS, pjAND[i]);

            // We need at least 600us between two accesses to the Bt48x
            // Doing some bogus register reads ought to do the trick
        ...
        for (i = 0 ; i < 128 ; i++)
        {
            CP_WRITE_REGISTER_BYTE(pjBase + BT482_OVRLAY_REGS, pjXOR[i]);
...
/****************************************************************************\
* SetTVP3026PointerShape -
\****************************************************************************/

ULONG SetTVP3026PointerShape(
SURFOBJ*    pso,
SURFOBJ*    psoMsk,
SURFOBJ*    psoColor,
XLATEOBJ*   pxlo,
...
FLONG       fl)
{
    ...
    UCHAR   ucByteWidth;
    UCHAR   ucTemp;
    BYTE    Tvp3026AndMask[64*8];
    BYTE    Tvp3026XorMask[64*8];

    ppdev  = (PDEV*) pso->dhpdev;
    pjBase = ppdev->pjBase;

    // Number of bytes per scanline of the masks.

    ucByteWidth = 8;

    // Init the AND and XOR masks.

    memset(Tvp3026AndMask, 0xff, 512);
    memset(Tvp3026XorMask, 0x00, 512);
    ...
    for (i = 0; i < cyAND; i++)
    {
        ...
        for (j = 0; j < cxAND; j++)
        {
            Tvp3026AndMask[i*ucByteWidth + j] = pjAND[j];
        }
    ...
    for (i = 0; i < cyXOR; i++)
    {
        ...
        for (j = 0; j < cxXOR; j++)
        {
            Tvp3026XorMask[i*ucByteWidth + j] = pjXOR[j];
        }
    ...
    pjAND = (BYTE*) Tvp3026AndMask;
    pjXOR = (BYTE*) Tvp3026XorMask;

    // The XOR mask goes to Plane 0, the AND mask to Plane 1.

    // Wait for VSYNC.  This will help with the famed TVP3026 noise.

    do {}
    while (!(CP_READ_REGISTER_BYTE(pjBase + HST_STATUS) & vsyncsts_MASK));

    // Disable the cursor, access bytes 000-0FF of cursor RAM.

    ppdev->bHwPointerActive = FALSE;

    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDIRECT_INDEX,TVP3026_I_CUR_CTL);
    ucTemp = CP_READ_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA);
    ucTemp &= ~(TVP3026_D_CURSOR_RAM_MASK | TVP3026_D_CURSOR_MASK);
    ucTemp |= (TVP3026_D_CURSOR_RAM_00 | TVP3026_D_CURSOR_OFF);
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA,ucTemp);

    // Start at byte 00 of Plane 0

    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_ADDR_WR,000);

    // Download the first 256 bytes of the XOR mask

    for (i = 0 ; i < 256 ; i++)
    {
        CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_DATA, pjXOR[i]);
    }
    ...
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDIRECT_INDEX,TVP3026_I_CUR_CTL);
    ucTemp = CP_READ_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA);
    ucTemp &= ~TVP3026_D_CURSOR_RAM_MASK;
    ucTemp |= TVP3026_D_CURSOR_RAM_01;
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA,ucTemp);
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_ADDR_WR,000);

    // Download the remaining 256 bytes of the XOR mask
    ...
    // Access bytes 200-2FF of cursor RAM.

    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDIRECT_INDEX,TVP3026_I_CUR_CTL);
    ucTemp = CP_READ_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA);
    ucTemp &= ~TVP3026_D_CURSOR_RAM_MASK;
    ucTemp |= TVP3026_D_CURSOR_RAM_10;
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA,ucTemp);
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_ADDR_WR,000);

    // Download the first 256 bytes of the AND mask

    for (i = 0 ; i < 256 ; i++)
    {
        CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_DATA, pjAND[i]);
    }
    ...
    ucTemp = CP_READ_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA);
    ucTemp &= ~TVP3026_D_CURSOR_RAM_MASK;
    ucTemp |= TVP3026_D_CURSOR_RAM_11;
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA, ucTemp);
    CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_ADDR_WR, 000);

    // Download the remaining 256 bytes of the AND mask.
...
FLONG       fl)
{
    ...
    case RAMDAC_TVP3026:
        ulRet = SetTVP3026PointerShape(pso, psoMsk, psoColor,
                                       pxlo, xHot, yHot, x, y, prcl, fl);
        break;
...
RECTL*      prcl)
{
    ...
    if (x == -1)
    {
        ...
        case RAMDAC_TVP3026:
            ...
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_X_LSB,TVP3026_OUT & 0xff);
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_X_MSB,(TVP3026_OUT >> 8));
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_Y_LSB,TVP3026_OUT & 0xff);
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_Y_MSB,(TVP3026_OUT >> 8));

            // Disable the cursor.
            // We must wait for a vsync here.
    ...
    else
    {
        ...
        case RAMDAC_TVP3026:
            ...
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_X_LSB, x & 0xff);
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_X_MSB, x >> 8);
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_Y_LSB, y & 0xff);
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_CUR_Y_MSB, y >> 8);
...
BOOL    bEnable)
{
    ...
    if (bEnable)
    {
        ...
        case RAMDAC_TVP3026:
            ...
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDIRECT_INDEX, TVP3026_I_CUR_CTL);
            byte = CP_READ_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA);
            byte &= ~TVP3026_D_CURSOR_MASK;
            byte |= TVP3026_D_CURSOR_OFF;
            CP_WRITE_REGISTER_BYTE(pjBase + TVP3026_INDEXED_DATA, byte);
