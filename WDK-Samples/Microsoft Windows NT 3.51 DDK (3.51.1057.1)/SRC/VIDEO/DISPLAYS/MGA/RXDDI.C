...
RXFUNC rxFuncs[] =
{
    ...
    RxAllocTexture,           // RXCMD_ALLOC_TEXTURE
...
ULONG RxEscape(RXESCAPE *pEsc)
{
    ...
    if (prxHdr->hrxSharedMem)
    {
        ...
        if (!pMemObj)
        {
            RXDBG_PRINT("RxEscape: Invalid handle for shared memory.");
            return FALSE;
        }
    ...
    else
    {
        ...
        if ((prxHdr->flags & RX_FL_MAP_MEM) &&
            ...
            RXDBG_PRINT("RxEscape: Invalid request in non-shared memory.");
            return FALSE;
        }
    }
    ...
    if (pEsc->pWnd && prxHdr->hrxRC)
    {
        ...
        if (!pRcObj)
        {
            RXDBG_PRINT("RxEscape: Invalid rendering context handle %x.",
                        prxHdr->hrxRC);
            return FALSE;
        }
        ...
        {
            RXDBG_PRINT("RxEscape: Invalid RC for this window.");
            return FALSE;
        }
	...
	// RXCMD_CREATE_CONTEXT:
        ...
        switch (pEsc->prxCmd->command) {
            ...
            case RXCMD_TEXTURE_HEAP:
                break;
            default:
                RXDBG_PRINT("RxEscape: Null rendering context invalid for command %d.", pEsc->prxCmd->command);
                return FALSE;
        }
    ...
    while ((BYTE*) pEsc->prxCmd <= (BYTE*) pEsc->pBufEnd - sizeof(RXCMD))
    {
        ...
        {
            RXDBG_PRINT("RxEscape: Command value %d invalid.", cmd);
            pEsc->retValue = FALSE;
...
void RxCreateContext(RXESCAPE *pEsc)
{
    ...
    RxEnable(NULL);     //!!!!!!! This belongs in driver init code...
    ...
    pRc->ppdev = pEsc->ppdev;

    HW_CREATE_RC(pEsc, pRc);
...
void RxGetInfo(RXESCAPE *pEsc)
{
    ...
    switch (rxGetInfo.infoType)
    {
        case RX_INFO_GLOBAL_CAPS:
            ...
            HW_GLOBAL_INFO(pEsc, &rxGetInfo, prxGlobalInfo);
        ...
        case RX_INFO_SURFACE_CAPS:
            ...
            HW_SURFACE_INFO(pEsc, &rxGetInfo, prxSurfaceInfo);
        ...
        case RX_INFO_SPAN_CAPS:
            ...
            HW_SPAN_CAPS(pEsc, &rxGetInfo, prxCaps);
        ...
        case RX_INFO_LINE_CAPS:
            ...
            HW_LINE_CAPS(pEsc, &rxGetInfo, prxCaps);
        ...
        case RX_INFO_INTLINE_CAPS:
            ...
            HW_INTLINE_CAPS(pEsc, &rxGetInfo, prxCaps);
        ...
        case RX_INFO_TRIANGLE_CAPS:
            ...
            HW_TRIANGLE_CAPS(pEsc, &rxGetInfo, prxCaps);
        ...
        case RX_INFO_QUAD_CAPS:
            ...
            HW_QUAD_CAPS(pEsc, &rxGetInfo, prxCaps);
...
void RxFillRect(RXESCAPE *pEsc)
{
    ...
    if (((rxFillRect.fillType & ~(RX_FILL_RECT_Z | RX_FILL_RECT_COLOR))) ||
        (!rxFillRect.fillType))
    {
        RXDBG_PRINT("RxFillRect: Invalid fill type (%x) specified.",
                     rxFillRect.fillType);
        ERROR_RETURN;
    }
    ...
    {
        RXDBG_PRINT("RxFillRect: RX_FILL_RECT_Z requested with z-buffer disabled.");
        ERROR_RETURN;
    }
...
void RxSetState(RXESCAPE *pEsc)
{
    ...
    switch (rxSetState.stateToChange)
    {
        ...
        case RX_Z_WRITE_ENABLE:
            HW_Z_WRITE_ENABLE(pEsc, pRc, rxSetState.newState[0]);
            break;
        ...
        case RX_TEXTURE:
            {
                ...
                if (pRc->texture == (RXHANDLE)rxSetState.newState[0])
                    return;

                if (rxSetState.newState[0]) {
                    pTexObj = GetPtrFromHandle((RXHANDLE)rxSetState.newState[0],
                                               RXHANDLE_TEXTURE);

                    if (!pTexObj) {
                        RXDBG_PRINT("RxSetState RX_TEXTURE: Invalid texture handle %x.",
                                     rxSetState.newState[0]);

                        SET_ERROR_CODE;
                        return;
                    }

                    HW_TEXTURE(pEsc, pTexObj->pTex, TRUE);
                    pRc->texture = (RXHANDLE)rxSetState.newState[0];

                } else {
                    pTexObj = GetPtrFromHandle(pRc->texture,
                                               RXHANDLE_TEXTURE);
                    pRc->texture = (RXHANDLE)NULL;

                    if (!pTexObj) {
                        RXDBG_PRINT("RxSetState RX_TEXTURE (NULL): Invalid texture handle (%x) in RC.",
                                     pRc->texture);
                        SET_ERROR_CODE;
                        return;
                    }

                    HW_TEXTURE(pEsc, pTexObj->pTex, FALSE);
                    pRc->texture = (RXHANDLE)NULL;
                }
            }
            break;
        ...
        case RX_VERTEX_TYPE:
            ...
            switch (pRc->vertexType = rxSetState.newState[0])
            {
                default:
                    SET_ERROR_CODE;
                    pRc->vertexType = RX_POINT;
                case RX_POINT:
                    pRc->coordStride = sizeof(RXPOINT);
                    break;
                case RX_POINTZ:
                    pRc->coordStride = sizeof(RXPOINTZ);
                    break;
                case RX_POINTZTEX:
                    pRc->coordStride = sizeof(RXPOINTZTEX);
                    break;
            }

            HW_VERTEX_TYPE(pEsc, pRc, rxSetState.newState[0]);

            pRc->vertexStride = pRc->coordStride + pRc->vertexColorStride;
            pRc->stripVertexStride = pRc->vertexStride + pRc->stripVertexSkip;

            break;

        case RX_SPAN_COLOR_TYPE:

            switch (pRc->spanColorType = rxSetState.newState[0])
            {
                default:
                    SET_ERROR_CODE;
                    pRc->spanColorType = RX_SPAN_COLOR_RGB;
                case RX_SPAN_COLOR_RGB:
                    pRc->spanColorStride = sizeof(RXCOLORREF);
                    break;
                case RX_SPAN_COLOR_RGBA:
                    pRc->spanColorStride = sizeof(RXCOLORREFA);
                    break;
                case RX_SPAN_COLOR_RGBAF:
                    pRc->spanColorStride = sizeof(RXCOLORREFAF);
                    break;
            }
        ...
        case RX_VERTEX_COLOR_TYPE:
            ...
            switch (pRc->vertexColorType = rxSetState.newState[0])
            {
                default:
                    SET_ERROR_CODE;
                    pRc->vertexColorType = RX_VERTEX_COLOR_PACKED;
                case RX_VERTEX_COLOR_PACKED:
                    pRc->vertexColorStride = sizeof(RXCOLOR);
                    break;
                case RX_VERTEX_COLOR_NONE:
                    pRc->vertexColorStride = 0;
                    break;
            }
        ...
        case RX_PRIMSTRIP_SKIP:

            if (rxSetState.newState[0] & 0x3) {
                SET_ERROR_CODE;
            }

            pRc->stripVertexSkip = rxSetState.newState[0] & ~3;
            pRc->stripVertexStride = pRc->vertexStride + pRc->stripVertexSkip;

            HW_PRIMSTRIP_SKIP(pEsc, pRc, rxSetState.newState[0]);

            break;

        case RX_PRIMLIST_SKIP:

            if (rxSetState.newState[0] & 0x3) {
                SET_ERROR_CODE;
            }

            pRc->listPointerSkip = rxSetState.newState[0] & ~3;

            pRc->lineListPointerStride = 2 * sizeof(char *) +
                                        pRc->listPointerSkip;

            pRc->triListPointerStride = pRc->lineListPointerStride +
                                        sizeof(char *);

            pRc->quadListPointerStride = pRc->triListPointerStride +
                                         sizeof(char *);

            HW_PRIMLIST_SKIP(pEsc, pRc, rxSetState.newState[0]);
...
//
// Handle the RXCMD_QUERY_TEXTURE_MEMORY command.
//****************************************************************************

void RxQueryTextureMemory(RXESCAPE *pEsc)
{
    RXRC *pRc = pEsc->pRc;
    RXQUERYTEXTUREMEMORY rxTextureMemory;
    VOID *pTexHeap = NULL;
    RXTEXHEAPOBJ *pTexHeapObj;
    RXTEXMEMSTATS *prxTexMemStats;
    void *pHeap;

    NEXT_COMMAND(rxTextureMemory);

    if (pEsc->cjOut < sizeof(RXTEXMEMSTATS))
    {
        RXDBG_PRINT("RxQureyTextureMemory: Output buffer too small.");
        ERROR_RETURN;
    }

    prxTexMemStats = (RXTEXMEMSTATS *)(pEsc->pvOut);

    if (rxTextureMemory.hrxTextureHeap) {
        if (!(pTexHeapObj = (RXTEXHEAPOBJ *)GetPtrFromHandle(rxTextureMemory.hrxTextureHeap,
                                            RXHANDLE_TEXTUREHEAP))) {
            RXDBG_PRINT("RxQueryTextureMemory: Invalid heap handle %x.",
                        rxTextureMemory.hrxTextureHeap);

            ERROR_RETURN;
        }
        pHeap = pTexHeapObj->pHeap;
    } else {
        pHeap = (void *)NULL;
    }

    HW_TEX_MEM_STATS(pEsc, prxTexMemStats, pHeap);
}
...
// RxTextureHeap()
//
// Handle the RXCMD_TEXTURE_HEAP command.
//****************************************************************************

void RxTextureHeap(RXESCAPE *pEsc)
{
    RXRC *pRc = pEsc->pRc;
    RXTEXTUREHEAP rxTextureHeap;
    VOID *pTexHeap = NULL;
    RXTEXHEAPOBJ *pTexHeapObj;

    NEXT_COMMAND(rxTextureHeap);

    switch (rxTextureHeap.action) {

        case RX_CREATE_TEXTURE_HEAP:

            if (!(pTexHeap = HW_CREATE_TEXTURE_HEAP(pEsc, &rxTextureHeap))) {
                RXDBG_PRINT("RxTextureHeap: Could not create new heap.");
                ERROR_RETURN;
            }

            if (!(pEsc->retValue = (ULONG)CreateTexHeapObj(pEsc, pTexHeap))) {
                HW_FREE_TEXTURE_HEAP(pTexHeap);
                RXDBG_PRINT("RxTextureHeap: Could not create new heap object.");
                ERROR_RETURN;
            }

            break;
        case RX_DELETE_TEXTURE_HEAP:
            if (!DestroyTexHeapObj(rxTextureHeap.hrxTextureHeap)) {
                RXDBG_PRINT("RxTextureHeap: Could not delete heap object %x.",
                            rxTextureHeap.hrxTextureHeap);
                ERROR_RETURN;
            }
            break;
        case RX_CLEAR_TEXTURE_HEAP:
            if (!(pTexHeapObj = (RXTEXHEAPOBJ *)GetPtrFromHandle(rxTextureHeap.hrxTextureHeap,
                                                    RXHANDLE_TEXTUREHEAP))) {
                RXDBG_PRINT("RxTextureHeap: Invalid heap handle %x.",
                            rxTextureHeap.hrxTextureHeap);

                ERROR_RETURN;
            }
            HW_CLEAR_TEXTURE_HEAP(pEsc, pTexHeapObj->pHeap);
            break;
        default:
            SET_ERROR_CODE;
            NONBATCHED_RETURN;
...
#define MAXTEXTURES     20  // Bigger than we should needs for mip-map levels

//****************************************************************************
// RxAllocTexture()
//
// Handle the RXCMD_ALLOC_TEXTURE command.
//****************************************************************************

void RxAllocTexture(RXESCAPE *pEsc)
{
    RXRC *pRc = pEsc->pRc;
    RXALLOCTEXTURE rxAllocTexture;
    RXTEXOBJ *pTexObj;
    RXTEXHEAPOBJ *pTexHeapObj;
    RXMEMOBJ *pMemObj;
    RXTEXTURE **ppTextures;
    RXTEXTURE *pTextures[MAXTEXTURES];

    LONG i;

    NEXT_COMMAND(rxAllocTexture);

    if (!(pTexHeapObj = (RXTEXHEAPOBJ *)GetPtrFromHandle(rxAllocTexture.hrxTextureHeap,
                                            RXHANDLE_TEXTUREHEAP))) {
        RXDBG_PRINT("RxAllocTexture: Invalid heap handle %x.",
                     rxAllocTexture.hrxTextureHeap);

        ERROR_RETURN;
    }

    pMemObj = (RXMEMOBJ *)GetPtrFromHandle(rxAllocTexture.hrxSharedMem,
                                           RXHANDLE_SHAREMEM);
    if (!pMemObj)
    {
        RXDBG_PRINT("RxAllocTexture: invalid shared-memory handle.");
        ERROR_RETURN;
    }

    if (rxAllocTexture.numTextures > MAXTEXTURES) {
        RXDBG_PRINT("RxAllocTexture: too many mip-map levels.");
        ERROR_RETURN;
    }

    ppTextures = (RXTEXTURE **)pEsc->prxCmd;

    // Advance the command buffer past the list of RXTEXTURE pointers:

    (char *)pEsc->prxCmd += (rxAllocTexture.numTextures * sizeof(VOID *));

    CHECK_BUFFER_SIZE(pEsc->prxCmd);

    // Convert the texture pointers to our address space:

    for (i = 0; i < (LONG)rxAllocTexture.numTextures; i++) {
        RXTEXTURE *pTexture;

        pTexture = pTextures[i] =
            (RXTEXTURE *)((char *)ppTextures[i] + pMemObj->addrOffset);
        CHECK_SHARED_MEM_RANGE((char *)pTexture, pMemObj);
        CHECK_SHARED_MEM_RANGE((char *)pTexture + sizeof(RXTEXTURE), pMemObj);
    }

    pEsc->retValue = (ULONG)CreateTexObj(pEsc, rxAllocTexture.hrxTextureHeap,
                                         pTexHeapObj->pHeap, pTextures);
...
// RxLoadTexture()
//
// Handle the RXCMD_LOAD_TEXTURE command.
//****************************************************************************

void RxLoadTexture(RXESCAPE *pEsc)
{
    RXRC *pRc = pEsc->pRc;
    RXLOADTEXTURE rxLoadTexture;
    RXTEXOBJ *pTexObj;
    RXMEMOBJ *pMemObj;
    RXTEXTURE **ppTextures;
    RXTEXTURE *pTextures[MAXTEXTURES];
    ...
    NEXT_COMMAND(rxLoadTexture);

    if (!(pTexObj = (RXTEXOBJ *)GetPtrFromHandle(rxLoadTexture.hrxTextureMem,
                                    RXHANDLE_TEXTURE))) {
        RXDBG_PRINT("RxLoadTexture: Invalid texture handle %x.",
                     rxLoadTexture.hrxTextureMem);
        ERROR_RETURN;
    }

    pMemObj = (RXMEMOBJ *)GetPtrFromHandle(rxLoadTexture.hrxSharedMem,
                                           RXHANDLE_SHAREMEM);
    if (!pMemObj)
    {
        RXDBG_PRINT("RxLoadTexture: invalid shared-memory handle.");
        ERROR_RETURN;
    }

    if (rxLoadTexture.numTextures > MAXTEXTURES) {
        RXDBG_PRINT("RxLoadTexture: too many mip-map levels.");
        ERROR_RETURN;
    }
    ...
    (char *)pEsc->prxCmd += (rxLoadTexture.numTextures * sizeof(VOID *));
    ...
    for (i = 0; i < (LONG)rxLoadTexture.numTextures; i++) {
        ULONG size;
        RXTEXTURE *pTexture;
        ...
        if (pTexture->driverPrivate & RX_IN_DEVICE_FORMAT) {
            size = pTexture->deviceFormatSize;
        } else {
            size = (pTexture->width + pTexture->border) *
                   (pTexture->height + pTexture->border);

            switch (pTexture->texelFormat) {
                case RX_TEXTURE_8888:
                    size *= 4;
                    break;
                case RX_TEXTURE_4444:
                case RX_TEXTURE_1555:
                case RX_TEXTURE_0565:
                    size *= 2;
                    break;
                case RX_TEXTURE_0332:
                    break;
                default:
                    RXDBG_PRINT("RxLoadTexture: invalid texture format.");
                    ERROR_RETURN;
            }
        }

        CHECK_SHARED_MEM_SIZE(size, pMemObj);
        CHECK_SHARED_MEM_RANGE((char *)pTexture + sizeof(RXTEXTURE) +
                               size - sizeof(ULONG), pMemObj);

    }

    pEsc->retValue = (ULONG)HW_LOAD_TEXTURE(pEsc, pTexObj->pTex, pTextures);
                ...
                ULONG cjOut, VOID *pvOut)
{
    ...
    switch (iEsc)
    {
        ...
        case RXFUNCS:
            ...
            if (!HW_INIT_RXESCAPE(&rxEscape))
                return FALSE;

            return RxEscape(&rxEscape);
...
//!!! double-check default values!!!
    ...
    pRc->spanColorStride = sizeof(RXCOLORREF);
    pRc->vertexColorStride = sizeof(RXCOLOR);
    pRc->stripVertexStride = pRc->vertexStride;
    pRc->stripVertexSkip = 0;
    pRc->listPointerSkip = 0;
    pRc->lineListPointerStride = 2 * sizeof(char *);
    pRc->triListPointerStride = 3 * sizeof(char *);
    pRc->quadListPointerStride = 4 * sizeof(char *);
...
BOOL DestroyRCObj(RXWINDOW *pWnd, HANDLE handle, RXHANDLE hrxRC)
{
    ...
    if (!pDrvObj) {
        RXDBG_PRINT("DestroyRCObj:  Couldn't unlock object.");
        return FALSE;
    }
    ...
    switch (pRcObject->type) {
        case RXHANDLE_RXRC:
            HW_DESTROY_RC(pRcObject->pObject);  // !!! Not sure about this
            return DestroyRC(pWnd, hrxRC);
        case RXHANDLE_TEXTURE:
            if (HW_FREE_TEXTURE(((RXTEXOBJ *)pDrvObj->pvObj)->pTex, FALSE)) {
                pDrvObj->pvObj = NULL;
                return (EngDeleteDriverObj((HDRVOBJ)handle, TRUE, FALSE) != 0);
            } else
                return TRUE;
        case RXHANDLE_TEXTUREHEAP:
        default:
            return (EngDeleteDriverObj((HDRVOBJ)handle, TRUE, FALSE) != 0);
    }
}
...
// FreeTexHeapObj()
//
// Engine callback for freeing the memory used for texture heaps.
//****************************************************************************

BOOL CALLBACK FreeTexHeapObj(DRIVEROBJ *pDrvObj)
{
    RXTEXHEAPOBJ *pTexHeapObj = (RXTEXHEAPOBJ *)pDrvObj->pvObj;

    HW_FREE_TEXTURE_HEAP(pTexHeapObj->pHeap);
    RxLocalFree((HLOCAL)pTexHeapObj);
...
// CreateTexHeapObj()
//
// Creates a handle for a new texture heap.
//****************************************************************************

RXHANDLE CreateTexHeapObj(RXESCAPE *pEsc, PVOID pTexHeap)
{
    RXTEXHEAPOBJ *newTexHeapObj;
    RXHANDLE handle;

    newTexHeapObj = (RXTEXHEAPOBJ *)RxLocalAlloc(0,sizeof(RXTEXHEAPOBJ));
    if (!newTexHeapObj)
        return NULL;

    handle = EngCreateDriverObj(newTexHeapObj, FreeTexHeapObj,
                                pEsc->ppdev->hdevEng);

    if (handle) {
        newTexHeapObj->hrxRC = NULL;
        newTexHeapObj->type = RXHANDLE_TEXTUREHEAP;
        newTexHeapObj->pHeap = pTexHeap;
        newTexHeapObj->size = sizeof(RXTEXHEAPOBJ);
    } else {
        RxLocalFree(newTexHeapObj);
        RXDBG_PRINT("CreateTexHeapObj: Could not create handle.");
    }
...
// DestroyTexHeapObj()
//
// Deletes the handle belonging to the specified texture heap.
//****************************************************************************

BOOL DestroyTexHeapObj(RXHANDLE hrxTexHeap)
{
    RXTEXHEAPOBJ *pTexHeapObj;

    if (!(pTexHeapObj = (RXTEXHEAPOBJ *)GetPtrFromHandle(hrxTexHeap,
                                            RXHANDLE_TEXTUREHEAP))) {
        RXDBG_PRINT("DestroyTexHeapObj: Invalid texture heap handle %x.",
                     hrxTexHeap);
        return FALSE;
    }
    return (EngDeleteDriverObj((HDRVOBJ)hrxTexHeap, TRUE, FALSE) != 0);
}
...
{
    RXTEXOBJ *pTexObj = (RXTEXOBJ *)pDrvObj->pvObj;

    if (pTexObj)
        HW_FREE_TEXTURE(pTexObj->pTex, TRUE);
    RxLocalFree((HLOCAL)pTexObj);
...
// CreateTexObj()
//
// Creates a handle for a new texture.
//****************************************************************************

RXHANDLE CreateTexObj(RXESCAPE *pEsc, RXHANDLE hrxTextureHeap, PVOID pHeap,
                      RXTEXTURE **ppTextures)
{
    RXTEXOBJ *newTexObj;
    RXHANDLE handle;

    newTexObj = (RXTEXOBJ *)RxLocalAlloc(0,sizeof(RXTEXOBJ));
    if (!newTexObj)
        return NULL;

    handle = EngCreateDriverObj(newTexObj, FreeTexObj,
                                pEsc->ppdev->hdevEng);

    if (handle) {
        newTexObj->hrxRC = (RXHANDLE)NULL;
        newTexObj->hrxTextureHeap = hrxTextureHeap;
        newTexObj->type = RXHANDLE_TEXTURE;
        newTexObj->size = sizeof(RXTEXOBJ);
        newTexObj->pTex = HW_ALLOC_TEXTURE(pEsc, handle, pHeap, ppTextures);

        if (!newTexObj->pTex) {
            RXDBG_PRINT("CreateTexObj: Could not create hardware texture.");
            EngDeleteDriverObj((HDRVOBJ)handle, TRUE, FALSE);
            return (RXHANDLE)NULL;
        }
    } else {
        RxLocalFree(newTexObj);
        RXDBG_PRINT("CreateTexObj: Could not create handle.");
    }
...
// DestroyTexObj()
//
// Deletes the handle belonging to the specified texture.
//****************************************************************************

BOOL DestroyTexObj(RXHANDLE hrxTex)
{
    RXTEXOBJ *pTexObj;

    if (!(pTexObj = (RXTEXOBJ *)GetPtrFromHandle(hrxTex,
                                    RXHANDLE_TEXTURE))) {
        RXDBG_PRINT("DestroyTexObj: Invalid texture handle %x.", hrxTex);
        return FALSE;
    }
    return (EngDeleteDriverObj((HDRVOBJ)hrxTex, TRUE, FALSE) != 0);
}
