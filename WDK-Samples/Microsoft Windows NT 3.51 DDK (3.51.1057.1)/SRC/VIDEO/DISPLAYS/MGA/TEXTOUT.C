...
*
* !!! Write some comments about the linear cache
*
...
CLIPOBJ*    pco)
{
    ...
    do {
        ...
        if (c != 0)
            vFillSolid(ppdev, c, ce.arcl, 0xf0f0, rbc, NULL);
...
PDEV*   ppdev)
{
    ...
    pcf = LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT, sizeof(CACHEDFONT));
    ...
    if (pcf != NULL)
    {
        ...
        // Note that we rely on LMEM_ZEROINIT to zero 'pgaChain' and
        // 'cjAlloc':
...
Restart:
    ...
    if (cjCachedGlyph > pcf->cjAlloc)
    {
        ...
        pga = LocalAlloc(LMEM_FIXED | LMEM_ZEROINIT, GLYPH_ALLOC_SIZE);
        if (pga == NULL)
        {
...
PDEV*   ppdev)
{
    ...
    if (poh != NULL)
    {
        ...
        ppdev->ulGlyphStart
            = (ppdev->ulYDstOrg + poh->y * ppdev->cxMemory) << cShift;

        ppdev->ulGlyphCurrent = ppdev->ulGlyphStart;

        ppdev->ulGlyphEnd
            = (ppdev->ulYDstOrg + (poh->y + poh->cy) * ppdev->cxMemory) << cShift;
    }
...
VOID vDisableText(PDEV* ppdev)
{
    ...
    // !!! Free any fonts?  Shouldn't have to.
}
