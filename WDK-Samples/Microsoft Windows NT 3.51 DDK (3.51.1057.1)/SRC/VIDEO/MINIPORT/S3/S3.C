...
// NOTE: BankMap can't be pageable
#endif
...

{
    ...
    case 0xE0: // Newer than 864/964
        ...
        case 0x8811:
            ...
            if ((jRevision & 0xf0) == 0x80) {
                ...
                //
                // Note: The 765 actually has all the capabilities of the
                //       868, except that it can't do New MM I/O on VLB
                //       systems.  We won't enable any of it's extended
        ...
        default:
            ...
            //
            // It's an S3 we don't recognize.  Assume it's backwards-
            // compatible:
            //
    ...
    if (hwDeviceExtension->Capabilities & CAPS_NEW_MMIO) {
        ...
        if (status != NO_ERROR) {
            ...
            VideoDebugPrint((0, "S3: Couldn't find room for new MM I/O window\n"));
            VideoDebugPrint((0, "S3: >> Status: 0x%lx\n", status));
        ...
        if (status != NO_ERROR) {
            ...
            VideoDebugPrint((0, "S3: Access Range conflict on remap\n"));
            return status;
        }
    }
    ...
    if ((jBus == 0x2) && (hwDeviceExtension->ChipID >= S3_866)) {
    ...
    if (hwDeviceExtension->ChipID <= S3_866) {
    ...
    hwDeviceExtension->OriginalRegPrimaryIndex = VideoPortReadPortUchar(CRT_DATA_REG);
    VideoPortWritePortUchar(CRT_ADDRESS_REG, hwDeviceExtension->FrequencySecondaryIndex);
    hwDeviceExtension->OriginalRegSecondaryIndex = VideoPortReadPortUchar(CRT_DATA_REG);
    ...
    case IOCTL_VIDEO_SET_CURRENT_MODE:
        ...
        if (hwDeviceExtension->BiosPresent) {
            ...
            if (FrequencyEntry->ScreenFrequency != 1) {
                ...
                if (hwDeviceExtension->RefreshSet) {
                    ...
                    VideoPortWritePortUchar(CRT_ADDRESS_REG, 0x52);
                    VideoPortWritePortUchar(CRT_DATA_REG,
                        hwDeviceExtension->OriginalRegPrimaryIndex);
                    VideoPortWritePortUchar(CRT_ADDRESS_REG,
                        hwDeviceExtension->FrequencySecondaryIndex);
                    VideoPortWritePortUchar(CRT_DATA_REG,
                        hwDeviceExtension->OriginalRegSecondaryIndex);
                }
            }
            ...
            if (status != NO_ERROR)
            {
                VideoDebugPrint((0, "S3: first int10 call FAILED\n"));
            }
            ...
            if ((status == NO_ERROR) && (biosArguments.Eax & 0xff00) == 0) {
                ...
                VideoDebugPrint((0, "S3: first int10 modeset failed\n"));
        ...
        if (status != NO_ERROR) {
            ...
            if (!hwDeviceExtension->BiosPresent) {
                ...
                VideoDebugPrint((0, "S3: Selected Fixed Frequency mode from int 10:\n"));
