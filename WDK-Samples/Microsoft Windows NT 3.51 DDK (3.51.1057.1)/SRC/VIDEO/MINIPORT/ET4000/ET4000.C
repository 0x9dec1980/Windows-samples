...

{
    ...
    if (hwDeviceExtension->ulChipID >= W32) {
        ...
        PHYSICAL_ADDRESS    paMmuBuffer = {MMU_BUFFER_MEMORY_ADDR, 0x0},
                            paAclRegs   = {MMU_MEMORY_MAPPED_REGS_ADDR, 0x0};


        pvMmuBuffer = VideoPortGetDeviceBase(hwDeviceExtension,
                                             paMmuBuffer,
        ...
        if (pvMmuBuffer == NULL) {
            ...
            VideoDebugPrint((0, "W32 FindAdapter - Fail to get MMU linear address\n"));
            return (ERROR_DEV_NOT_EXIST);

        }

        hwDeviceExtension->pvMmuBuffer = pvMmuBuffer;

        pvAclRegs = VideoPortGetDeviceBase(hwDeviceExtension,
                                           paAclRegs,
        ...
        if (pvAclRegs == NULL) {
            ...
            VideoDebugPrint((0, "W32 FindAdapter - Fail to get ACL linear address\n"));
            return (ERROR_DEV_NOT_EXIST);

        }

        hwDeviceExtension->pvAclRegs = pvAclRegs; ;

        hwDeviceExtension->pvW32MmuAperture2 = (PUCHAR) pvMmuBuffer + APERTURE_2_OFFSET;
    }
...

{
    ...
    case IOCTL_VIDEO_SHARE_VIDEO_MEMORY:
        ...
        status = VideoPortMapBankedMemory(hwDeviceExtension,
                                          ...
                                          &inIoSpace,
                                          &virtualAddress,
                                          hwDeviceExtension->PhysicalFrameLength,   // bank size
        ...
        if (status != NO_ERROR)
        {
            VideoDebugPrint((0, "VideoPortMapBankedMemory returned (%d), see wininc-winerror.h\n", status));
        }
    ...
    case IOCTL_VIDEO_QUERY_PUBLIC_ACCESS_RANGES:
        ...
        if (RequestPacket->OutputBufferLength <
            sizeof(W32_ADDRESS_MAPPING_INFORMATION))
        {
            status = ERROR_INSUFFICIENT_BUFFER;
        }
        else
        {
            PW32_ADDRESS_MAPPING_INFORMATION pW32AddrMapInfo;

            PHYSICAL_ADDRESS PhysicalAddress;
            ULONG            ulLength;
            ULONG            ulInIoSpace;
            PVOID            *ppvVirtualAddress;

            pW32AddrMapInfo =
               (PW32_ADDRESS_MAPPING_INFORMATION) RequestPacket->InputBuffer;

            RequestPacket->StatusBlock->Information =
                                  sizeof(W32_ADDRESS_MAPPING_INFORMATION);

            PhysicalAddress.LowPart  = pW32AddrMapInfo->ulPhysicalAddress;
            PhysicalAddress.HighPart = 0;
            ulLength                 = pW32AddrMapInfo->ulLength;
            ulInIoSpace              = pW32AddrMapInfo->ulInIoSpace;
            ppvVirtualAddress        = &(pW32AddrMapInfo->pvVirtualAddress);
    ...
    case IOCTL_VIDEO_MAP_VIDEO_MEMORY:
        ...
        if (hwDeviceExtension->CurrentMode->banktype == MemMgrBanking) {

            VideoDebugPrint((2, "VgaStartIO - MapVideoMemory used kernel banked memory\n"));
            ...
            VideoDebugPrint((2, "VgaStartIO - MapVideoMemory didn't use kernel banked memory\n"));
