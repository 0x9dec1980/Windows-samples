...
VP_STATUS
CirrusGetCapabilities(
    PHW_DEVICE_EXTENSION HwDeviceExtension,
    PCIRRUS_CAPABILITIES pCaps,
    PULONG OutputSize
    );
...
#pragma alloc_text(PAGE,CirrusGetCapabilities)
...

{
    ...
    else
    {
        VideoDebugPrint((1, "Cirrus!VgaFindAdapter: "
                            "ConfigInfo->AdapterInterfaceType != PCIBus\n"));
        hwDeviceExtension->LinearMode = FALSE;
    }
    ...
    if (ConfigInfo->AdapterInterfaceType == Internal) {
        ...
        status = VideoPortGetDeviceData(HwDeviceExtension,
                                        VpControllerData,
                                        &CirrusGetDeviceDataCallback,
                                        ConfigInfo);
    ...
    //
#ifdef _ALPHA_
        VgaIoBase.HighPart = 0;
        VgaIoBase.LowPart = VGA_BASE_IO_PORT;
...

{
    ...
    case IOCTL_VIDEO_FREE_PUBLIC_ACCESS_RANGES:
     ...
     case IOCTL_CIRRUS_GET_CAPABILITIES:
        ...
        if (RequestPacket->OutputBufferLength < sizeof(CIRRUS_CAPABILITIES))
        {
            status = ERROR_INSUFFICIENT_BUFFER;
            break;
        }
        status = CirrusGetCapabilities(HwDeviceExtension,
                            (PCIRRUS_CAPABILITIES) RequestPacket->OutputBuffer,
                            &RequestPacket->StatusBlock->Information);
        break;

     case IOCTL_CIRRUS_SET_DISPLAY_PITCH:

        if (RequestPacket->InputBufferLength < sizeof(USHORT) )
        {
            status = ERROR_INSUFFICIENT_BUFFER;
            break;
        }

        status = CirrusSetDisplayPitch (HwDeviceExtension,
                  (PUSHORT) RequestPacket->InputBuffer);
        break;

    case IOCTL_CIRRUS_PRIVATE_BIOS_CALL:

        // BUGBUG !!! Display drivers should not do this.

        VideoDebugPrint((1, "VgaStartIO - CirrusPrivateIOCTL\n"));

        status = VideoPortInt10(HwDeviceExtension,
                                (PVIDEO_X86_BIOS_ARGUMENTS) RequestPacket->OutputBuffer);

        RequestPacket->StatusBlock->Information = sizeof(VIDEO_X86_BIOS_ARGUMENTS);

        VideoDebugPrint((1, "VgaStartIO - END CirrusPrivateIOCTL\n"));
        break;
    ...
    if (retvalue)
    {
             ...
             (HwDeviceExtension->ChipType == CL5434_6))
         {
             ...
             if ((HwDeviceExtension->ChipType == CL5434_6) &&
                 (SystemBusSelect == 0x10))      // Bits 5:3 from SR17 == 100
             {
...
#else  // for MIPS NEC machine only
...
--*/
{
    ...
    if (VideoPortReadPortUchar(CRTCDataPort) & 0x20)
      ...
      if (temp1 == 1)   // this means panel connected is 800x600
         {
         return (panel | panel8x6); // will support either 800x600 or 640x480
         }
      else
         {
         return panel;
    ...
    )
{
   ...
   pCaps->fl = 0;     // Default to nothing enabled
   if (HwDeviceExtension->VideoPointerEnabled)
      {
      pCaps->fl |= CL_ALLOW_HW_CURSOR;
      }
   if ((ChipType == CL543x) || (ChipType == CL754x)) // All 543x & 754x do BLTs
      {
      pCaps->fl |= CL_BLT_SUPPORT;
      }
   ...
   if (HwDeviceExtension->AllowOpaqueText)
      {
      pCaps->fl |= CL_ALLOW_OPAQUE_TEXT;
      }
   if (HwDeviceExtension->LinearMode)
      {
      pCaps->fl |= CL_LINEAR_MODE;
      }
   pCaps->ulMemSize = HwDeviceExtension->AdapterMemorySize;
   pCaps->ulOffscreenMemTop = HwDeviceExtension->AvailOffscreenMemLimit;
   pCaps->ulChipId = HwDeviceExtension->ChipRevision;

   *OutputSize = pCaps->size = sizeof(CIRRUS_CAPABILITIES);
...
CirrusSetDisplayPitch (
   PHW_DEVICE_EXTENSION HwDeviceExtension,
   PUSHORT pRequestedPitchInBytes
   )
{
    PUCHAR CRTCAddressPort, CRTCDataPort;
    USHORT RequestedPitchInBytes =  *pRequestedPitchInBytes;
    USHORT PitchInQuadWords = RequestedPitchInBytes >> 3;
    ULONG  VisPitchInBytes = (HwDeviceExtension->CurrentMode->hres *
                             HwDeviceExtension->CurrentMode->bitsPerPlane) / 8 ;

    VideoDebugPrint((1, "CirrusSetDisplayPitch - Entry\n"));

    if ((RequestedPitchInBytes < VisPitchInBytes) ||
        ((RequestedPitchInBytes & 7) != 0) ||
        (PitchInQuadWords > 0x1FF) )
    {
        VideoDebugPrint((0, "CirrusSetDisplayPitch - Exit (ERROR_INVALID_PARAMETER)\n"));
        return ERROR_INVALID_PARAMETER;
    }
    ...
    HwDeviceExtension->CurrentMode->wbytes = RequestedPitchInBytes;
   ...
   )
{
    ...
    while (*pDeviceId != 0)
    {
        ...
        if (status == NO_ERROR)
        {
            ...
            PCIAccessRanges[2].RangeStart  = AccessRanges[0].RangeStart;
            PCIAccessRanges[2].RangeLength = AccessRanges[0].RangeLength;
    ...
    )
{
    ...
    switch( hwDeviceExtension->ChipType )
    {
        ...
        case CL542x: pwszChipType =    L"CL 542x";
                     cbString = sizeof(L"CL 542x");
                     break;

        case CL543x: pwszChipType =    L"CL 543x";
...

{
    ...
    case VpControllerData:
        ...
        if ( VideoPortCompareMemory(identifier, L"CIRRUS ON BOARD", 32) != 32)
        {
            return ERROR_DEV_NOT_EXIST;
        }
        else
