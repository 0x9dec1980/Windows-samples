...
#if DBG
                DbgPrint("PS: ERROR: SC_IDENTITY = 24bpp\n");
#endif
            }
...
#if DBG
        DbgPrint("PS: ERROR: XlateColor Passed non 16/24/32=%ld", Srcbpp);
#endif
        break;
    }
}
...

{
	...
	// 4/15/95 -davidx
	// Make sure the source rectangle is not empty. DDI spec
	// says the source rectangle is also well ordered and
	// guaranteed not empty. That apparently is not the case.
...
#if DBG
        DbgPrint("PS: BeginImageEx() failed on DoSourceCopy\n");
#endif
        return(FALSE);
    }
    ...
    if (SCFlags) {
        ...
        if (!(pbBuf = (LPBYTE)HeapAlloc(pDev->hheap, 0, cbDstScanLine))) {

#if DBG
            DbgPrint("PS: Allocate LShift/XlateColor Buffer Failed\n");
#endif
            return(FALSE);
        }
...

{
    ...
    while (bMoreClipping) {
        ...
        //
        // DoSourceCopy does not know how to use prclClip, so don't
        // give it one.
        //

        bOk = PSSourceCopy(pDev, psoSrc, szlSrc, pptlSrc, prclDest, pxlo);
...

{
    ...
    if (pdev->dwFlags & PDEV_PSHALFTONE) {
        ...
        if (szlSrc.cx > psoSrc->sizlBitmap.cx - pptlSrc->x) {

            szlSrc.cx = psoSrc->sizlBitmap.cx - pptlSrc->x;
        }

        if (szlSrc.cy > psoSrc->sizlBitmap.cy - pptlSrc->y) {

            szlSrc.cy = psoSrc->sizlBitmap.cy - pptlSrc->y;
        }
...

{
    ...
    if (pdev->dwFlags & PDEV_PSHALFTONE) {
        ...
        if (psoMask) {
            ...
            DbgPrint("PS: Printer Halftone ignored psoMask\n");
        }

        DbgPrint("PS: Printer Halftone ignored COLORADJUSTMENT\n");
#endif
...

{
    ...
    cy1 = cy;
#else
    PSPrintf(pdev, "%f %f translate %f %f scale\n",
               X72DPI(x),
               Y72DPI(y),
               ((cx * PS_FIX_RESOLUTION) + (pdev->psdm.dm.dmPrintQuality / 2)) /
               pdev->psdm.dm.dmPrintQuality,
               ((cy * PS_FIX_RESOLUTION) + (pdev->psdm.dm.dmPrintQuality / 2)) /
               pdev->psdm.dm.dmPrintQuality);
    cy1 = -cy;
#endif
...
PFILTER         pFilter;
{
    ...
    PSPrintf(pdev, "%l %l %l %l %l %l %l %l %b %b %l beginimage\n",
            sizlSrc.cx, sizlSrc.cy, ulSrcFormat, cbSrcWidth,
