...

{
    ...
    for (disks = 0; disks < configurationInformation->CdRomCount; disks++) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            //
            // Check for Lion Optics drive
            //

            if ((inquiryDataPtr[8] =='S') &&
                 (inquiryDataPtr[9] =='A') &&
                 (inquiryDataPtr[10]=='N') &&
                 (inquiryDataPtr[11]=='Y') &&
                 (inquiryDataPtr[12]=='O') &&
                ...
                MmLockPagableCodeSection((PVOID)CdAudioLionOpticsDeviceControl);
                deviceExtension->Active = CDAUDIO_LION_OPTICS;
                inquiryDataPtr[30] = (UCHAR)0;
                CdDump((3,
                        "CdAudioInitialize: %s is a %s drive.\n",
                        deviceNameBuffer,
                        &inquiryDataPtr[8]));
...

{
    ...
    switch( deviceExtension->Active ) {
        ...
        case CDAUDIO_LION_OPTICS:
            status = CdAudioLionOpticsDeviceControl( DeviceObject, Irp );
            break;
...

{
    ...
    CdDump ((3,"CdAudioLionOpticsDeviceControl: IoControl %x.\n",
                currentIrpStack->Parameters.DeviceIoControl.IoControlCode));

    //
    // This device needs support only for IOCTL_SEEK_AUDIO_MSF
    //

    control = currentIrpStack->Parameters.DeviceIoControl.IoControlCode;
    if (currentIrpStack->Parameters.DeviceIoControl.IoControlCode == IOCTL_CDROM_SEEK_AUDIO_MSF) {
        ...
        lba = MSF_TO_LBA(inputBuffer->StartingM, inputBuffer->StartingS, inputBuffer->StartingF);

        cdb->SEEK.LogicalBlockAddress[0] = ((PFOUR_BYTE)&lba)->Byte3;
        cdb->SEEK.LogicalBlockAddress[1] = ((PFOUR_BYTE)&lba)->Byte2;
        cdb->SEEK.LogicalBlockAddress[2] = ((PFOUR_BYTE)&lba)->Byte1;
        cdb->SEEK.LogicalBlockAddress[3] = ((PFOUR_BYTE)&lba)->Byte0;
        ...
        if (!NT_SUCCESS(status)) {
            ...
            CdDump((1,
                    "CdAudioLionOpticsDeviceControl: Seek failed (0x%08lX)\n",
                    status ));
        }
    } else {
        return CdAudioSendToNextDriver( DeviceObject, Irp );
