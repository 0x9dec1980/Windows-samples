...

{
    ...
    if (!currentIrpStack->Parameters.Read.Length) {
        //
        // In this case the caller has used a zero length buffer
        // so we succeed the read while doing nothing
        //
...

{
    ...
    if (!currentIrpStack->Parameters.Write.Length) {
        //
        // In this case the caller has used a zero length buffer
        // so we succeed the write while doing nothing
        //
        ntStatus = STATUS_SUCCESS;
        goto exit;
    }
