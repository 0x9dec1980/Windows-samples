...
Routine Description:
    ...
    This routine will erase and/or format the tape.
...

{
    ...
    if (!NT_SUCCESS(status)) {
        DebugPrint((1,"TapeErase: rewind, SendSrb unsuccessful\n"));
        return status;
    }
...
Routine Description:
    ...
    minicartridge QIC tape drive associated with "DeviceObject". From time
    to time, the drive paramater set of a given drive is variable. It will
...

{
    ...
    PREAD_BLOCK_LIMITS_DATA     blockLimitsModeSenseBuffer;
    PINQUIRYDATA                inquiryBuffer;
    ...
    blockLimitsModeSenseBuffer = ExAllocatePool(NonPagedPoolCacheAligned,
                                                sizeof(READ_BLOCK_LIMITS_DATA));

    if (!blockLimitsModeSenseBuffer) {
        DebugPrint((1,"TapeGetDriveParameters: insufficient resources (blockLimitsModeSenseBuffer)\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    RtlZeroMemory(blockLimitsModeSenseBuffer, sizeof(READ_BLOCK_LIMITS_DATA));
    ...
    status = ScsiClassSendSrbSynchronous(DeviceObject,
                                         &srb,
                                         blockLimitsModeSenseBuffer,
                                         sizeof(READ_BLOCK_LIMITS_DATA),
                                         FALSE);

    if (NT_SUCCESS(status)) {
        tapeGetDriveParams->MaximumBlockSize =   blockLimitsModeSenseBuffer->BlockMaximumSize[2];
        tapeGetDriveParams->MaximumBlockSize += (blockLimitsModeSenseBuffer->BlockMaximumSize[1] << 8);
        tapeGetDriveParams->MaximumBlockSize += (blockLimitsModeSenseBuffer->BlockMaximumSize[0] << 16);

        tapeGetDriveParams->MinimumBlockSize =   blockLimitsModeSenseBuffer->BlockMinimumSize[1];
        tapeGetDriveParams->MinimumBlockSize += (blockLimitsModeSenseBuffer->BlockMinimumSize[0] << 8);
    }

    ExFreePool(blockLimitsModeSenseBuffer);
    ...
    switch (driveID) {
        case CTMS_3200:
            ...
            tapeGetDriveParams->ReportSetmarks = reportSetmarks;
...
Routine Description:
    ...
    minicartridge QIC tape drive associated with "DeviceObject". Tape
...
Routine Description:
    ...
    This routine "sets" the "drive parameters" of the minicartridge
    QIC drive associated with "DeviceObject": e.g., Set Mark reporting
    enable/disable.
...
Routine Description:
    ...
    This routine "sets" the "media parameters" of the minicartridge
    QIC drive associated with "DeviceObject". Tape media must be present
...
{
    if (RtlCompareMemory(InquiryData->VendorId,"CONNER  ",8) == 8) {

        if (RtlCompareMemory(InquiryData->ProductId,"CTMS  3200",10) == 10) {
            return CTMS_3200;
        }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"EXABYTE ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"EXB-2502",8) == 8) {
            return EXABYTE_2505;
        }
    }
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"TANDBERG",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId," TDC 3500",9) == 9) {
            return TDC_3500;
        }
