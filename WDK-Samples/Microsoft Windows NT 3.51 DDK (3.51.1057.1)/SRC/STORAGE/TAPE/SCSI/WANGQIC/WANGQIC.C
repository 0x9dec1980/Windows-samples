...

{
    ...
    //
    //  QFA mode has only 2 partitions
    //  Partition 1 -- used as a directory partition
    //  Partition 0 -- used as a data partition
    //
    ...
    //
    //  Now find out what sort of drive this is.
    //
...

{
    ...
    blockDescripterModeSenseBuffer = ExAllocatePool(NonPagedPoolCacheAligned,
                                                    sizeof(MODE_PARM_READ_WRITE_DATA));

    if (!blockDescripterModeSenseBuffer) {
        DebugPrint((1,"TapeGetDriveParameters: insufficient resources (blockDescripterModeSenseBuffer)\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    RtlZeroMemory(blockDescripterModeSenseBuffer, sizeof(MODE_PARM_READ_WRITE_DATA));
    ...
    status = ScsiClassSendSrbSynchronous(DeviceObject,
                                         &srb,
                                         blockDescripterModeSenseBuffer,
                                         sizeof(MODE_PARM_READ_WRITE_DATA),
                                         FALSE);

    if (NT_SUCCESS(status)) {

        mediumType       =  blockDescripterModeSenseBuffer->ParameterListHeader.MediumType;
        densityCode      =  blockDescripterModeSenseBuffer->ParameterListBlock.DensityCode;
        tapeBlockLength  =  blockDescripterModeSenseBuffer->ParameterListBlock.BlockLength[2];
        tapeBlockLength += (blockDescripterModeSenseBuffer->ParameterListBlock.BlockLength[1] << 8);
        tapeBlockLength += (blockDescripterModeSenseBuffer->ParameterListBlock.BlockLength[0] << 16);
    ...
    ExFreePool(blockDescripterModeSenseBuffer);
            ...
            switch (densityCode) {
                case QIC_XX:
                    switch (mediumType) {
                        ...
                        case DC9500:
                        case DC9500SL:
                            tapeGetDriveParams->DefaultBlockSize = tapeBlockLength;
...

{
    ...
    switch (driveID) {
        ...
        case WANGTEK_9500DC:
            ...
            readPositionBuffer = ExAllocatePool(NonPagedPoolCacheAligned,
                                                sizeof(TAPE_POSITION_DATA));

            if (!readPositionBuffer) {
                DebugPrint((1,"TapeGetPosition: insufficient resources (readPositionBuffer)\n"));
                return STATUS_INSUFFICIENT_RESOURCES;
            }

            RtlZeroMemory(readPositionBuffer, sizeof(TAPE_POSITION_DATA));
            ...
            cdb->READ_POSITION.BlockType = (type == TAPE_LOGICAL_POSITION)?
                                           SETBITOFF : SETBITON;
            ...
            if (type == TAPE_PSEUDO_LOGICAL_POSITION) {
                tapeBlockAddress =
...
Routine Description:
    ...
    This routine "sets" the "drive parameters" of the Wangtek drive
...

{
    ...
    switch (driveID) {
        ...
        case WANGTEK_9500DC:
            ...
            if (tapeBlockLength != tapeSetMediaParams->BlockSize) {
        ...
        case WANGTEK_9500DC:
            if (method == TAPE_LOGICAL_BLOCK) {
                ...
                switch (tapeSetPosition->Partition) {
                    ...
                    case DATA_PARTITION:
                        if (tapeData->CurrentPartition != NOT_PARTITIONED) {
            ...
            if (method == TAPE_PSEUDO_LOGICAL_BLOCK) {
            ...
            switch (method) {
                ...
                case TAPE_PSEUDO_LOGICAL_BLOCK:
                    ...
                    cdb->LOCATE.BTBit = (method == TAPE_LOGICAL_BLOCK)?
                                         SETBITOFF : SETBITON;
                    ...
                    srb.TimeOutValue = 480;
                    if ((method == TAPE_PSEUDO_LOGICAL_BLOCK) &&
                        (physPosition.SpaceBlockCount != 0)) {
                        ...
                        cdb->SPACE_TAPE_MARKS.NumMarksMSB =
                             (UCHAR)((physPosition.SpaceBlockCount >> 16) & 0xFF);
                        cdb->SPACE_TAPE_MARKS.NumMarks =
                             (UCHAR)((physPosition.SpaceBlockCount >> 8) & 0xFF);
                        cdb->SPACE_TAPE_MARKS.NumMarksLSB =
                             (UCHAR)(physPosition.SpaceBlockCount & 0xFF);
                        srb.TimeOutValue = 480;
            ...
            switch (method) {
                ...
                case TAPE_SPACE_END_OF_DATA:
                    if ((driveID == WANGTEK_5150) || (driveID == WANGTEK_5360))  {
                        ...
                        if (!NT_SUCCESS(status)) {
                           DebugPrint((1,"TapeSetPosition: method = rewind before space EOD, SendSrb unsuccessful\n"));
                           return status;
                        }
...

{
    if (RtlCompareMemory(InquiryData->VendorId,"WANGTEK ",8) == 8) {
        ...
        if (RtlCompareMemory(InquiryData->ProductId,"9500   ",7) == 7) {
            return WANGTEK_9500;
        }

        if (RtlCompareMemory(InquiryData->ProductId,"9500 DC",7) == 7) {
