...

{
    ...
    switch (modeSenseBuffer->MediumPartPage.PSUMBit) {
        case 1:
            partitionSize[0].QuadPart *= 1024;
            partitionSize[1].QuadPart *= 1024;
            break;

        case 2:
            partitionSize[0].QuadPart *= 1048576;
            partitionSize[1].QuadPart *= 1048576;
            break;
    }
    ...
    switch (driveID) {
        ...
        case IOMEGA_DAT4000:
            ...
            if (NT_SUCCESS(status)) {
                ...
                remaining[0].QuadPart *= 1024;
                remaining[1].QuadPart *= 1024;
                remaining[0].QuadPart += remaining[1].QuadPart;

                capacity[0].QuadPart *= 1024;
                capacity[1].QuadPart *= 1024;
