...
#define CONNER_CTMI_3200    4  // aka the Conner Ctmi 3200
...

{
    ...
    if (driveID == CONNER_CTMI_3200) {

        tapeGetDriveParams->MaximumBlockSize = tapeBlockLength;
        tapeGetDriveParams->MinimumBlockSize = tapeBlockLength;
    ...
    switch (type) {
        ...
        case TAPE_ABSOLUTE_POSITION:
            ...
            if (driveID == CONNER_CTMI_3200) {
                ...
                if (NT_SUCCESS(status)) {
                    tapePosBuffer = (PTAPE_POSITION_DATA) absoluteBuffer;
...

{
    ...
    } else if (RtlCompareMemory(InquiryData->VendorId,"CONNER  ",8) == 8) {

        if (RtlCompareMemory(InquiryData->ProductId,"CTMI  3200",10) == 10) {
            return CONNER_CTMI_3200;
        }
