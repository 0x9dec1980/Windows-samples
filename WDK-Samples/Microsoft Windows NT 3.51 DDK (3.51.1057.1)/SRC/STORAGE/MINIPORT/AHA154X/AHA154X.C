...
A154xDetermineInstalled(
    ...
    IN OUT PCURRENT_CONTEXT CurrentContext,
    OUT PBOOLEAN Again
    );
...

{
    ...
    currentContext.AdapterCount = 0;
    currentContext.BiosBaseMask = 0;

    isaStatus = ScsiPortInitialize(DriverObject, Argument2, &hwInitializationData, &currentContext);
    ...
    currentContext.AdapterCount = 0;
    currentContext.BiosBaseMask = 0;
    mcaStatus = ScsiPortInitialize(DriverObject, Argument2, &hwInitializationData, &currentContext);
...

{
    ...
    if ((*ConfigInfo->AccessRanges)[0].RangeLength != 0) {
        ...
        *Again = FALSE;
        ...
        while (AdapterAddresses[CurrentContext->AdapterCount] != 0) {

            //
            // Get next base address.
            //

            baseIoAddress = (PBASE_REGISTER)(ioSpace + AdapterAddresses[CurrentContext->AdapterCount]);
            HwDeviceExtension->BaseIoAddress = baseIoAddress;
            ioPort = AdapterAddresses[CurrentContext->AdapterCount];

            //
            // Update the Adapter count
            //

            (CurrentContext->AdapterCount)++;
            ...
            if ((portValue & ~0x29) == IOP_SCSI_HBA_IDLE) {
                ...
                if (!AdaptecAdapter(HwDeviceExtension,
                                    ioPort,
                ...
                //
                                      ...
                                      ConfigInfo,
                                      AdapterAddresses[(CurrentContext->AdapterCount) - 1])) {
    ...
    if (inboundData == 0x41) {
        ...
        for (i = 0; i < 6; i ++) {
            ...
            if (j == 16) {
                ...
                //
                // Found the BIOS. Set up ConfigInfo->AccessRanges if this range hasn't already
                // been claimed by another aha-154x adapter.
                //

                if (!(CurrentContext->BiosBaseMask & (1 << i))) {
                    ...
                    DebugPrint((1,
                               ...
                               0xC8000 + i * 0x4000 ));
                    CurrentContext->BiosBaseMask |= (1 << i);
                    break;

                } else {

                    DebugPrint((1,
                               "A154xClaimBiosSpace: 154XB BIOS address = %lX already claimed.\n",
                               0xC8000 + i * 0x4000 ));
                }
            }
        }
        ...
        if ((inboundData == 0x44) || (inboundData == 0x45)) {
            ...
            if (inboundData != 0x07 && inboundData != 0x06) {
                ...
                i = (ULONG)((~inboundData & 0x07) - 2);
                baseBIOSAddress += i * 0x4000;
                ...
                if (!(CurrentContext->BiosBaseMask & (1 << i))) {
                    ...
                    DebugPrint((1,
                               "A154xClaimBiosSpace: 154XC BIOS address = %lX already claimed.\n",
                               baseBIOSAddress ));
                }
            }
        }
