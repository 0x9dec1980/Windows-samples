...

{
    ...
    if (path = ExAllocatePool(PagedPool, pathLength)) {
        ...
        // Doing this lets SMC 661, and 662 work.  On hardware that
        //
        // If a Japanese "3 mode" floppy driver wants to support multiple
        // PCs by single floppy driver binary, it is recommended that its
        // setup script add registry variable "ModelName" to floppy driver
        // registry parameter, and decide here on which type of PC the
        // driver is installed, then it can pass this information to
        // FlInitializeController as the original code does for "model30".
        // 
...
#ifdef JAPAN
    controllerData->VariableSpeed = TRUE;
#endif // JAPAN
...
//    if ( NT_SUCCESS( ntStatus ) ) {
        ...
        if ( controllerData->VariableSpeed ) {

            ChangeResult = FlChangeMotorSpeed( DisketteExtension, FALSE );
        }
//    }
    ...
    if ( NT_SUCCESS( ntStatus ) ) {
        if ( controllerData->VariableSpeed ) {
            ChangeResult = FlChangeMotorSpeed( DisketteExtension, TRUE );
...

{
    ...
    //
#ifdef JAPAN
    ntStatus = FlCheckBootSector(DisketteExtension);

    if (!NT_SUCCESS( ntStatus ) && !mediaTypesExhausted) {
        FloppyDump( FLOPDBGP,("Floppy: TRY_AGAIN_WITH_LOWER_MEDIA\n"));

        goto TRY_AGAIN_WITH_LOWER_MEDIA;
    }
...
Arguments:
    ...
    Context -  tells FlChangeMotorSpeed() in which context it has been
    called.
