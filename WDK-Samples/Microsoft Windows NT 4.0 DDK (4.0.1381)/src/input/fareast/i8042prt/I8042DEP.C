...
#endif // defined(WINDOWS_FE) && defined(_X86_)
#endif
...
#if defined(WINDOWS_FE) && defined(_X86_) // DriverEntry()
    //
...
#ifdef WINDOWS_FE // DriverEntry(

    //
    // Upfates device identifier.
    // 
    {
        ...
        OverrideDeviceIdentifier(
            configuration->OverrideKeyboardIdentifier,
...

{
...
#ifdef WINDOWS_FE // I8xServiceParameters()
    UNICODE_STRING defaultKeyboardIdentifier;
    UNICODE_STRING overrideKeyboardIdentifier;
#endif // WINDOWS_FE
    NTSTATUS status = STATUS_SUCCESS;
    PWSTR path = NULL;
#ifdef WINDOWS_FE // I8xServiceParameters()
    ...
    if (NT_SUCCESS(status)) {
...
#ifdef WINDOWS_FE // I8xServiceParameters()
        //
        // Initialize override keyboard identifier to NULL.
        //
        RtlInitUnicodeString(
        ...
        parameters[15].Name = L"OverrideKeyboardIdentifier";
        parameters[15].EntryContext = &overrideKeyboardIdentifier;
        parameters[15].DefaultType = REG_SZ;
        parameters[15].DefaultData = &defaultKeyboardIdentifier;
        parameters[15].DefaultLength = 0;
#endif // WINDOWS_FE
...
#ifdef WINDOWS_FE // I8xServiceParameters()
        wcscpy(configuration->OverrideKeyboardIdentifier,L"\0");
#endif // WINDOWS_FE
    } else {
...
#ifdef WINDOWS_FE // I8xServiceParameters()
        if (overrideKeyboardIdentifier.Buffer) {
            wcscpy(configuration->OverrideKeyboardIdentifier,
                   overrideKeyboardIdentifier.Buffer);
            RtlFreeUnicodeString(&overrideKeyboardIdentifier);
        }
#endif // WINDOWS_FE
    }
