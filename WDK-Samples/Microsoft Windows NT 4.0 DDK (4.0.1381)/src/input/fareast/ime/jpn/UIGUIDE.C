...
void PASCAL PaintGuide( HWND hGuideWnd , HDC hDC, LPPOINT lppt, DWORD dwPushedGuide)
{
    ...
    if (hIMC = (HIMC)GetWindowLong(hSvrWnd,IMMGWL_IMC))
    {
        ...
        if (dwLevel = ImmGetGuideLine(hIMC,GGL_LEVEL,NULL,0))
        {
            ...
            // 
            // since ImmGetGuideLine accepts GGL_PRIVATE, the return
            // value of ImmGetGuideLine is ALWAYS the size of the buffer
            // in bytes.
            //
            if ( dwSize > 0 ) {
                pGLStr = LocalAlloc( LMEM_FIXED, dwSize+sizeof(TCHAR) );
                if ( pGLStr != NULL ) {
                    COLORREF rgb = 0;
                    ...
                    if ( ImmGetGuideLine(hIMC,GGL_STRING,pGLStr,dwSize) > 0 )
                    {
                        SetTextColor(hDC,rgb);
                        SetBkMode(hDC,TRANSPARENT);
                        TextOut(hDC,0,nCyCap,pGLStr, dwSize / sizeof(TCHAR));
                    }
                    LocalFree( pGLStr );
                }
            }
