    ...
    LPPOINT        lpptWnd)         // the composition window position
{
    ...
    if (uEsc == 0) {
        ...
        if (ptNew.x + lpImeL->xCandWi > sImeG.rcWorkArea.right) {
            // exceed screen width
        ...
        if (lpptWnd->y >= ptCurPos.y) {
            ...
            if (ptNew.y + lpImeL->yCandHi > sImeG.rcWorkArea.bottom) {
                // exceed screen high
                ptNew.y = sImeG.rcWorkArea.bottom - lpImeL->yCandHi;
            }
        } else {
            ptNew.y = lpptWnd->y + lpImeL->yCompHi - lpImeL->yCandHi;

            if (ptNew.y < sImeG.rcWorkArea.top) {
                ptNew.y = sImeG.rcWorkArea.top;
            }
        }
    } else if (uEsc == 1) {
        if (lpptWnd->x >= ptCurPos.x) {
            ...
            if (ptNew.x + lpImeL->xCandWi > sImeG.rcWorkArea.right) {
                // exceed screen width
                ptNew.x = sImeG.rcWorkArea.right - lpImeL->xCandWi;
                fAdjust = TRUE;
            }
        } else {
            ptNew.x = lpptWnd->x + lpImeL->xCompWi - lpImeL->xCandWi;

            if (ptNew.x < sImeG.rcWorkArea.left) {
                // exceed screen width
                ptNew.x = sImeG.rcWorkArea.left;
                fAdjust = TRUE;
            }
        }
    ...
    LPPOINT lpptCandWnd)            // the position
{
    ...
    } else if (lpptCandWnd->x + lpImeL->xCandWi > sImeG.rcWorkArea.right) {
        lpptCandWnd->x = sImeG.rcWorkArea.right - lpImeL->xCandWi;
    } else {
    }

    if (lpptCandWnd->y < sImeG.rcWorkArea.top) {
        lpptCandWnd->y = sImeG.rcWorkArea.top;
    } else if (lpptCandWnd->y + lpImeL->yCandHi > sImeG.rcWorkArea.bottom) {
        lpptCandWnd->y = sImeG.rcWorkArea.bottom - lpImeL->yCandHi;
    } else {
    }
