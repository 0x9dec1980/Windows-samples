    ...
    HWND hWnd)
{
    ...
    for (i = 0; i < nIMEs; i++) {
        ...
        if (lRet * sizeof(WCHAR) / sizeof(TCHAR) >
            sizeof(lpRegWordStructTmp->szReading) - sizeof(TCHAR)) {
...
#ifdef UNICODE
    lpImeLinkRegWord->szEudcCodeString[0] = (TCHAR)uCode;
#else
    ...
    UINT   uLen)
{
    ...
    for (i = 0; i < nLayout; i++) {
        ...
        fRet = ImmEscapeA(hKL, (HIMC)NULL, IME_ESC_IME_NAME, szImeName);
    ...
    UINT          uCodePage)
{
    ...
    if (uCodePage == UNICODE_CP) {
        LPTSTR lpszMethodName;

        lpszMethodName = (LPTSTR)lpIsvUsrDic->achMethodName;

        for (i = 0; i < sizeof(lpIsvUsrDic->achMethodName);
            i += sizeof(TCHAR)) {
            szStr[i / sizeof(TCHAR)] = *lpszMethodName++;
        }

        hKL = MatchImeName(szStr, sizeof(lpIsvUsrDic->achMethodName) /
            sizeof(TCHAR));
    } else {
        LPSTR lpszMethodName, lpszStr;

        lpszMethodName = lpIsvUsrDic->achMethodName;
        lpszStr = (LPSTR)szStr;

        for (i = 0; i < sizeof(lpIsvUsrDic->achMethodName); i++) {
            *lpszStr++ = *lpszMethodName++;
        }

        hKL = MatchImeNameA((LPSTR)szStr, sizeof(lpIsvUsrDic->achMethodName) /
            sizeof(CHAR));
    }
#else
    for (i = 0; i < sizeof(lpIsvUsrDic->achMethodName); i++) {
    ...
    HWND hWnd)
{
    ...
    if (dwSize != dwFileSize) {
        ...
        MessageBox(hWnd, szMessage, szTitle, MB_OK);
    } else if (*(LPDWORD)lpIsvUsrDic->idUserCharInfoSign != SIGN_CWIN) {
        // != CWIN
        LoadString(hAppInst, IDS_CWINSIGN_TITLE, szTitle, iTitle);
        LoadString(hAppInst, IDS_CWINSIGN_MSG, szMessage, iMessage);

        MessageBox(hWnd, szMessage, szTitle, MB_OK);
    } else if (*(LPDWORD)((LPBYTE)lpIsvUsrDic->idUserCharInfoSign +
