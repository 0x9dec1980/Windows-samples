      ...
      && (wCharCode != TEXT(' '))) {
		...
		if(sImeL.dwRegImeIndex == INDEX_GB)
		{
            ...
            if((lpImcP->bSeq[2] == TEXT('?'))) {
		        ...
		        wCode = GBEngine(lpImcP);
#ifdef UNICODE
				MultiByteToWideChar(CP_ACP, NULL, &wCode, 2, ResaultStr, sizeof(ResaultStr)/sizeof(TCHAR));
				ResaultStr[1] = TEXT('\0');
#else
...
#else
					WideCharToMultiByte(936, NULL, &wCode, 1, &xCode, 2, NULL, NULL);
				    // add this string into candidate list
    ...
    WORD            wCode)
{
...
#ifdef UNICODE
	{
		...
		MultiByteToWideChar(CP_ACP, NULL, &wInCode, 2, &wUnicode, 1);
		*(LPUNAWORD)((LPBYTE)lpCandList + lpCandList->dwOffset[
