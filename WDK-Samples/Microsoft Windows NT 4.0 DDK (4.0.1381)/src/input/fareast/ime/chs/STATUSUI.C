	...
	HWND     hUIWnd)
{
	...
	lppt->x = (short)sImeG.rcWorkArea.left;
    } else if (lppt->x + sImeG.xStatusWi > sImeG.rcWorkArea.right) {
	lppt->x = (short)(sImeG.rcWorkArea.right - sImeG.xStatusWi);
    }

    if (lppt->y < sImeG.rcWorkArea.top) {
	lppt->y = (short)sImeG.rcWorkArea.top;
    } else if (lppt->y + sImeG.yStatusHi > sImeG.rcWorkArea.bottom) {
	lppt->y = (short)(sImeG.rcWorkArea.bottom - sImeG.yStatusHi);
    }
    ...
    if(sImeG.IC_Trace) {
		...
		if(lppt->x + sImeG.xStatusWi + Comp_CandWndLen > sImeG.rcWorkArea.right) {
    ...
    LPPOINT lpptCursor)
{
	...
	} else 
	{
		...
		if (lpbKeyState[VK_CAPITAL] & 1)		//Keep original logic, toggle off only
		{
