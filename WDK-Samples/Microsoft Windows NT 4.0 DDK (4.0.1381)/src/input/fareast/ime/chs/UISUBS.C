    ...
    case WM_COMMAND:
		...
		case IDM_OPTGUD:
			...
			TCHAR szOPTGUDHlpName[MAXSTRLEN];
  			szOPTGUDHlpName[0] = 0;
	      	lstrcpy((LPTSTR)szOPTGUDHlpName, (LPTSTR)TEXT("WINIME.HLP"));
		...
		case IDM_IMEGUD:
			...
			//COMBO_IME has only one IME help file
  			lstrcpy((LPTSTR)szIMEGUDHlpName, TEXT("WINGB.HLP"));
#else //COMBO_IME
#ifdef GB
  			lstrcpy((LPTSTR)szIMEGUDHlpName, TEXT("WINGB.HLP"));
#else
		...
		case IDM_VER:
	        ...
	        DialogBox(hInst, TEXT("IMEVER"), lpIMC->hWnd, ImeVerDlgProc);
