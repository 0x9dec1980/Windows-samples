    ...
    LPPOINT lpptWnd)            // the composition window position
{
    ...
    if(lpptWnd->y + ptFontHi + sImeG.yCandHi <= sImeG.rcWorkArea.bottom) {
    	lpptWnd->y += ptFontHi;
	} else {
    ...
    LPPOINT        lpptCandWnd)            // the caret position
{
    ...
    *(LPPOINT)&rcUIRect = *lpptCandWnd;
    ...
    if (rcUIRect.left < sImeG.rcWorkArea.left) {
        rcUIRect.left = sImeG.rcWorkArea.left;
    } else if (rcUIRect.left + sImeG.xCandWi > sImeG.rcWorkArea.right) {
        rcUIRect.left = sImeG.rcWorkArea.right - sImeG.xCandWi;
    } else {
    }

    if (rcUIRect.top < sImeG.rcWorkArea.top) {
        rcUIRect.top = sImeG.rcWorkArea.top;
    } else if (rcUIRect.top + sImeG.yCandHi > sImeG.rcWorkArea.bottom) {
        rcUIRect.top = sImeG.rcWorkArea.bottom - sImeG.yCandHi;
    } else {
    }
    ...
    *(LPPOINT)&rcUIRect = *lpptCandWnd;
    ...
    if (uEsc & 0x0001) {
        ...
        *lpptCandWnd = *(LPPOINT)&rcUIRect;
        ...
        if (rcUIRect.top < sImeG.rcWorkArea.top) {
        } else if (rcUIRect.top + sImeG.yCandHi > sImeG.rcWorkArea.right) {
        } else {
            if (rcUIRect.left < sImeG.rcWorkArea.left) {
                rcUIRect.left = sImeG.rcWorkArea.left;
            } else if (rcUIRect.left + sImeG.xCandWi > sImeG.rcWorkArea.right) {
    ...
    HWND hUIWnd)
{
    ...
    if (lpUIPrivate->hCandWnd) {
        ...
        lpUIPrivate->hCandWnd = CreateWindowEx(WS_EX_WINDOWEDGE|WS_EX_DLGMODALFRAME,
            szCandClassName, NULL,WS_POPUP|WS_DISABLED,ptWnd.x,ptWnd.y,sImeG.xCandWi, 
			sImeG.yCandHi,hUIWnd, (HMENU)NULL, hInst, NULL);
    ...
    if (sImeG.IC_Trace) {
	    ...
	    for (i = 0; dwStart < dwEnd; dwStart++, i++) {
	       ...
	       // QW/GB info
	       {
			 ...
			 AbSeq[0] = wCode;
			 AbSeq[1] = TEXT('\0');
			 WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, AbSeq, 1, GbSeq, sizeof(GbSeq), NULL, NULL);
