...
#else
    			retCode = RegCreateKeyEx(hKeyCurrVersion, szImeName, 0,
    				NULL, REG_OPTION_NON_VOLATILE,	KEY_ALL_ACCESS	, NULL, &hKeyGB, NULL);
#endif //COMBO_IME
    ...
    LPVOID  lpData)            // mode of dialog
{
    ...
    case IME_CONFIG_GENERAL:
        DialogBox(hInst, TEXT("ImeSet"), hAppWnd, ImeSetDlgProc);
        break;
    default:
        return (FALSE);
        break;
...
{        
	...
	WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK, &wCode, 1, &AreaCode, 2, NULL, NULL);
	wCode = AreaCode;
#endif
    ...
    UINT            uBufLen)
{
...
#ifndef UNICODE
	{
		...
		MultiByteToWideChar(CP_ACP, 0, (LPCSTR)&wCode, sizeof(WORD), szWideStr, sizeof(szWideStr));
