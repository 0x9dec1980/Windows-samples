...
RECTL*  prclSrc)
{
    ...
    ASSERTDD(((ULONG) pjSrc & 3) == 0, "Must dword align source");
...
FLATPTR fpVidMem)   // Surface for which we're requesting flip status;
                    //   0 indicates status of last flip, regardless of what
                    //   surface it was.
{
    BYTE*       pjBase;
    DWORD       dwScanLine;
    ...
    if (ppdev->flipRecord.bFlipFlag)
    {
        ...
        if (dwScanLine >= ppdev->flipRecord.dwScanLine)
        {
            ...
            if ((fpVidMem == 0) || (fpVidMem == ppdev->flipRecord.fpFlipFrom))
            {
...
PDD_FLIPDATA lpFlip)
{
    ...
    ddRVal = ddrvalUpdateFlipStatus(ppdev, 0);
...
PDD_GETFLIPSTATUSDATA lpGetFlipStatus)
{
    ...
    lpGetFlipStatus->ddRVal = ddrvalUpdateFlipStatus(ppdev, 0);
            ...
            ((wWidth == ppdev->cxScreen) && (wHeight == ppdev->cyScreen)))
        {
            ...
            if (poh != NULL)
            {
                ...
                if ((wWidth  != ppdev->cxScreen) ||
                    ...
                    (fpVidMem < 0x400000))
                {
                    lpSurface->dwReserved1  = (DWORD) poh;
