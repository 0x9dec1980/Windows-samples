...

{
    ...
    // causes the HAL screen to become corrupted.  So, lets
    // read it once into a buffer, and then use that from
    // here on out.
    //
    // In the future, we may want to find a way to get a
    // pointer to the HAL's cached copy of the ROM, and use
    // that.
    //

    Wait_VSync(HwDeviceExtension);
    VideoPortMoveMemory(romShadow, romAddress, MAX_ROM_SCAN);

    romAddress = romShadow;
