...
DWORD   ProcessPJLString(PINIPORT, CHAR *, LPDWORD);
...
Arguments:
    hPort           : Port handle
    cmd             : Command buffer
    bDirectToPort   : Should the WritePort on port monitor be called directly?
...
--*/
{
    ...
    if (bDirectToPort)
        ...
        dwRet = PJLMonWritePort(hPort, cmd, cbWrite, &cbWritten);
