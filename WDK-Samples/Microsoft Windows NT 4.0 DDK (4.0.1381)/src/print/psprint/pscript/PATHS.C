...
FLONG     flOptions;
{
    ...
    gsaved = bDoClipObj(pdev, pco, NULL, &rclBounds);
    ...
    if (!ps_patfill(pdev, pso, flOptions, pbo, pptlBrushOrg, MixToRop4(mix), &rclBounds,
                    FALSE, TRUE))
        return(FALSE);
...
//!!! this needs to be fixed!!! -kentse.
    ...
    if (!ps_patfill(pdev, pso, flOptions, pboFill, pptlBrushOrg, MixToRop4(mixFill),
