...
Abstract:
    ...
    This module contrains sample prototype for the NT 4.0 common Property
    Sheet user interface


Author:

    15-Jul-1996 Mon 13:43:46 created  -by-  Daniel Chou (danielc)
...
Author:
    ...
    15-Jul-1996 Mon 17:25:17 created  -by-  Daniel Chou (danielc)
...

{
    ...
    if (PageSetupDlg(&PSD)) {
        ...
        if (Ok) {
            ...
            pCPSUISample->DPHdr.fMode          = (DM_IN_BUFFER      |
                                                    DM_IN_PROMPT    |
                                                    DM_OUT_BUFFER);
...
Author:
    ...
    15-Jul-1996 Mon 15:19:44 created  -by-  Daniel Chou (danielc)
...

{
    ...
    case PROPSHEETUI_REASON_INIT:
        ...
        if (!(pCPSUISample = (PCPSUISAMPLE)LocalAlloc(LPTR,
                                                      sizeof(CPSUISAMPLE)))) {

            return(-1);
        }

        pPSUIInfo->UserData   = (DWORD)pCPSUISample;
        ...
        case IDM_DOCPROP_TVTEST:
            ...
            if (GetDefaultPrinter(pCPSUISample)) {

                InsPI.cbSize  = sizeof(INSERTPSUIPAGE_INFO);
                InsPI.Type    = PSUIPAGEINSERT_DLL;
                InsPI.Mode    = INSPSUIPAGE_MODE_FIRST_CHILD;
                InsPI.dwData1 = (DWORD)szWinSpool;
                InsPI.dwData2 = (DWORD)szDocPropSheets;
                InsPI.dwData3 = (DWORD)(&pCPSUISample->DPHdr);
        ...
        case IDM_DOCPROP_TVTEST:
            ...
            if (SetupComPropSheetUI(&(pCPSUISample->CPSUI))) {
                ...
                InsPI.Mode    = INSPSUIPAGE_MODE_FIRST_CHILD;
                InsPI.dwData1 = (DWORD)&(pCPSUISample->CPSUI);
                InsPI.dwData2 =
                InsPI.dwData3 = 0;
...
Author:
    ...
    15-Jul-1996 Mon 13:29:03 created  -by-  Daniel Chou (danielc)
    ...
    case WM_INITMENUPOPUP:
        ...
        if (!HIWORD(lParam)) {
            ...
            CheckMenuItem((HMENU)wParam,
                          IDM_PERMISSION,
                          MF_BYCOMMAND | ((UpdatePermission) ? MF_CHECKED :
                                                               MF_UNCHECKED));
            CheckMenuItem((HMENU)wParam,
                          IDM_USESTDABOUT,
                          MF_BYCOMMAND | ((UseStdAbout) ? MF_CHECKED :
                                                          MF_UNCHECKED));
        }
    ...
    case WM_COMMAND:
        ...
        case IDM_DOCPROP_TVTEST:
            ...
            CPSUIDBG(DBG_WINMAINPROC,
                     ("CommonPropertySheetUI()=%ld, Result=%ld",
                        Ret, Result));
...
Author:
    ...
    15-Jul-1996 Mon 13:21:30 created  -by-  Daniel Chou (danielc)
...
Author:
    ...
    15-Jul-1996 Mon 13:17:57 created  -by-  Daniel Chou (danielc)
...
Author:
    ...
    15-Jul-1996 Mon 13:13:41 created  -by-  Daniel Chou (danielc)
