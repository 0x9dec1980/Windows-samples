...

{
    ...
    if (pfo->flFontType & DEVICE_FONTTYPE) {
        ...
        // Get the DEVFONT structure for the specified font.
        ...
        pfont = (PDEVFONT)LISTOBJ_FindIndexed((PLISTOBJ)
                    pdev->hppd->pFontList, pfo->iFace);

        // Copy the font name to the buffer.

        CopyStringA(pbuffer, (pfont ? pfont->pName : ""), cb);
#else // DBCS
        PNTFM   pntfm;
