...
HSEMAPHORE  hGlyphSetSemaphore;
HSEMAPHORE  hFontNameSemaphore;
#endif // DBCS
...
#if defined(DBCS) // FE font
    hGlyphSetSemaphore = CREATESEMAPHORE();
    hFontNameSemaphore = CREATESEMAPHORE();
#endif // DBCS
...
#if defined(DBCS) // Output Glyph Prefer Option
	wCurOutGlyPreferOpt = GetOutGlyPreferOption(hPrinter);
    pdev->hppd = PpdCreate(EngGetPrinterDataFileName(hdev), pdev->hModule, 
		                   wCurOutGlyPreferOpt);
#ifdef DBG
	{
		char   strBuf[256];
		
        CopyUnicode2Str(strBuf,EngGetPrinterDataFileName(hdev), wcslen(EngGetPrinterDataFileName(hdev)) );
        strBuf[wcslen(EngGetPrinterDataFileName(hdev))] = '\0';
		DBGMSG1(DBG_LEVEL_WARNING, "Data File Name is %s \n", strBuf);
    }
#endif
#else // DBCS
...
#if defined(DBCS) // FE font
    DELETESEMAPHORE(hGlyphSetSemaphore);
    DELETESEMAPHORE(hFontNameSemaphore);
#endif // DBCS
...
{
    if (strcmp(pFontObj->pName, (PCSTR)pDevFontName) == EQUAL_STRING
            || strcmp(((PDEVFONT)pFontObj)->pXlation, (PCSTR)pDevFontName)
            == EQUAL_STRING) {
        return FALSE;
    }
...

{
    ...
    if (pdev->pTTSubstTable == NULL) {
...
#if defined(DBCS) // FE font
        {
            ...
            CHAR DevFontName[MAX_FONTNAME];

            for (pwstr = pdev->pTTSubstTable; *pwstr != L'\0';) {
                pwstrsave = pwstr;
                pwstr += wcslen(pwstr) + 1;
                CopyUnicode2Str(DevFontName, pwstr, sizeof(DevFontName));

                if (LISTOBJ_Enum((PLISTOBJ)pdev->hppd->pFontList,
                        DevFontNameProc, (DWORD)DevFontName) == NULL) {
...

{
    ...
    if (pdev->cDeviceFonts > 0) {
       ...
       /* Set DDI default font to Courier, 100 pixels tall */
		...
		// Courier is not appropriate for DBCS locales.  It is better let the
		// GDI choose whatever font with appropriate charset.

		wcscpy((PWSTR)mydevinfo.lfDefaultFont.lfFaceName, L"");

		if (!TranslateCharsetInfo((DWORD *)GetACP(), &ci, TCI_SRCCODEPAGE))
			ci.ciCharset = ANSI_CHARSET;
		mydevinfo.lfDefaultFont.lfCharSet = ci.ciCharset;
...
#if defined(DBCS)
    // make sure its charset is ANSI
	   mydevinfo.lfAnsiVarFont.lfCharSet = ANSI_CHARSET;
#endif // DBCS
...
#if defined(DBCS)
    // make sure its charset is ANSI
	   mydevinfo.lfAnsiFixFont.lfCharSet = ANSI_CHARSET;
#endif // DBCS
