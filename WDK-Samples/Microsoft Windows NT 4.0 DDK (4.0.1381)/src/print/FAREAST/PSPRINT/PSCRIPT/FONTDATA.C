...
Routine Description:
    ...
    Returns character width information, supporting fixed-pitch DBCS
    part fonts.

--*/

{
    PIFIMETRICS pifi = (PIFIMETRICS)
            ((PBYTE) pntfm + pntfm->ntfmsz.loIFIMETRICS);
    LONG lWidth;

    if (hg > 0xff) {
        if (pifi->flInfo & FM_INFO_DBCS_FIXED_PITCH)
            lWidth = (LONG)pifi->fwdMaxCharInc;
        else
            lWidth = (LONG)pifi->fwdAveCharWidth;
    }
    else {
...
#if defined(DBCS) // FE font
            lWidth = CalcurateCharWidth(pntfm, hg);
#else // DBCS
            lWidth = (LONG) pntfm->ausCharWidths[hg];
#endif // DBCS
...
#if defined(DBCS) // FE font
            // A fonts with fixed-pitch DBCS part has 0 external leading.
            if (pifi->flInfo & FM_INFO_DBCS_FIXED_PITCH)
                fxExtLeading = 0;
#endif // DBCS
...
#if defined(DBCS) // FE font
        // We have to be strict on setting lD value.  DBCS fonts have
        // two (halfwidth and fullwidth) character width in single font
        // even if when it has FIXED_PITCH attribute.
...

{
    ...
    {
#if defined(DBCS) // FE font
        LONG lWidth = CalcurateCharWidth(pntfm, *phg++);
#else // DBCS
