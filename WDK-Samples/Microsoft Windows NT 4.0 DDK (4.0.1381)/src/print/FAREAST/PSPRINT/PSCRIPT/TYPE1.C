...
#ifdef DBCS
LONG  GlyphAlreadyDefined(DEVDATA *,DLFONT *, DWORD);
LONG  FindDeviceGlyphIndex(DEVDATA *, DLFONT *,DWORD);
VOID  PrintBitmapInChunks(PDEVDATA , GLYPHBITS * );
#endif
...
//	DBGMSG(DBG_LEVEL_WARNING, "Enter DownloadType1Char\n");
...
//	DBGMSG(DBG_LEVEL_WARNING, "Enter DownloadType1Font\n");
    ...
    // we retrieve bounding box information from the IFIMETRICS.
    // counting each glyph is more accurate to guess which is the
    // real minimum bounding-box, but for DBCS fonts which contains
    // more than 10,000 glyphs it is a palin to waiting minutes
    // (literally!) for this loop to finish.
  
      rcl.left = pifi->rclFontBox.left;
      rcl.top = pifi->rclFontBox.top;
      rcl.right = pifi->rclFontBox.right;
      rcl.bottom = pifi->rclFontBox.bottom;
  
      XFORMOBJ_bApplyXform(pxo, XF_LTOFX, 2, &rcl, &rcfx1);

    // flip y coordinate (PS vs Win)
 
      ptlBL.x = FXTOL(rcfx1.xLeft);
      ptlBL.y = -FXTOL(rcfx1.yBottom);
      ptlTR.x = FXTOL(rcfx1.xRight);
      ptlTR.y = -FXTOL(rcfx1.yTop);
	}
...
#if defined(DBCS)
    // a PS font can hold upto 255 glyphs plus "/.notdef"
    psprintf(pdev, "%d EA\n", min(255, cGlyphs - 1));
#else // DBCS
    psprintf(pdev, "%d EA\n", cGlyphs-1);
#endif // DBCS
...
#ifdef KKBUGFIX // font name

#define ISPSSPECIAL(c) \
    ((c) == '(' || (c) == ')' || (c) == '<' || (c) == '>' || \
    (c) == '[' || (c) == ']' || (c) == '{' || (c) == '}' || \
    (c) == '/' || (c) == '%')
#endif
...
#ifndef KKBUGFIX // font name
        } else if ((*pstrName == '(') || (*pstrName == ')')) {

#else
		} else if (ISPSSPECIAL(*pstrName)) {
#endif
...
{
#ifdef KKBUGFIX // iUniq

    // There are cases where one same TrueType typeface is realized with
    // different pfo->iUniq, so you may have name conflict (defining two
    // different downloaded font with a same name).  To avoid this, you
    // have to generate download font filename based on whateve you use
    // to dsitingish between two donloaded fonts.
    ...
    pfontname += strlen ( pfontname );
    if (( pfo->flFontType & TRUETYPE_FONTTYPE ) && boutline ) {
        SPRINTF( pfontname, "1%x", pfo->iTTUniq );   
		pfontname +=strlen( pfontname);

// Because Bold simulated font should share the same data with regular font, 
// No need to create another downloading font.
...
/*		if ((pfo->flFontType & FO_SIM_BOLD) != 0) {
        ...
        if  ((pfo->flFontType & FO_SIM_ITALIC) != 0) {
			SPRINTF( pfontname, "I");   
			pfontname +=strlen( pfontname);
        }
    }
    else {
        SPRINTF( pfontname, "3%x",
            pfo->iUniq );
    }
    pfontname += strlen ( pfontname );
    *pfontname = '\0'; 
...
PrintBitmapInChunks(
    ...
    GLYPHBITS *pgb )
{
    LONG cWidth, cLength, cDelta;
    BYTE *pjCurPos, *pjMaxPos;
    FILTER filter;
    LONG cChunks;

    // there are cases where the bitmap size exceeds 64k and this hits
    // PS limitation.  in such occasion, we output the bitmap in chunks.

    cWidth = ((pgb->sizlBitmap.cx + 7) >> 3);
    cLength = pgb->sizlBitmap.cy;
    if ((cDelta = cWidth * cLength) > 65535)
        cDelta = 65535;
    pjCurPos = pgb->aj;
    pjMaxPos = pjCurPos + cWidth * cLength;

    // always define this as a procedure (since it will always be
    // used as the parameter to imagemask.)

    psputs( pdev, "\n" );
    cChunks = 0;

    for (; pjCurPos < pjMaxPos; pjCurPos += cDelta, cChunks++)
    {
        if ( pjCurPos + cDelta > pjMaxPos)
            cDelta = (LONG)(pjMaxPos - pjCurPos);

        if ( ! Level2Device(pdev->hppd) ) {

            psputs(pdev, "<" );
            psputhex(pdev, cDelta, pjCurPos);
            psputs(pdev, ">\n" );
        }
        else {
            ...
            if ( filter.dwFilterFlags & FILTER_FLAG_HEX )
            {
                psprintf( pdev, "\ncurrentfile %d string readhexstring\n", cDelta );
            }
            else
            {
                psprintf(pdev, "\ncurrentfile %s %s %d string readstring\n",
                    ((filter.dwFilterFlags & FILTER_FLAG_ASCII85) ? "/ASCII85Decode filter" : ""),
                    ((filter.dwFilterFlags & FILTER_FLAG_RLE) ? "/RunLengthDecode filter" : ""),
                    cDelta );
            }

            FILTER_WRITE( &filter, pjCurPos, cDelta );
...
//	DBGMSG(DBG_LEVEL_WARNING, "Enter DownloadType3Char\n");
...
//--------------------------------------------------------------------
// VOID StopAddingCharsToFont(
// PDEVDATA    pdev,
// BOOL boutline )
//
// Stop adding cahracters to the font, clean up the operand stack.
// History:
//   xx-Nov-1995    -by-    Takashi Matsuzawa (takashim)
//  Broke out of AddCharsToFont.
//--------------------------------------------------------------------

VOID
StopAddingCharsToFont(
DEVDATA *pdev,
BOOL boutline
)
{
    ...
    if (boutline) {
        eexecPrintf(pdev, "end end mark currentfile closefile\n");
...
//	DBGMSG(DBG_LEVEL_WARNING, "Enter DownloadType3Font\n");
    ...
    // We retrieve bounding box information from the IFIMETRICS.
    // counting each glyph is more accurate to guess which is the
    // real minimum bbox, but for DBCS fonts which contains more
    // than 10,000 glyphs it is a palin to wait minutes waiting
    // for this loop to finish.
        ...
        XFORMOBJ_bApplyXform(pxo, XF_LTOFX, 2, &rcl, &rcfx);
  
        ptlTL.x = FXTOL(rcfx.xLeft);
        ptlTL.y = -FXTOL(rcfx.yTop); // flip y coordinate (PS vs Win)
        ptlBR.x = FXTOL(rcfx.xRight);
        ptlBR.y = -FXTOL(rcfx.yBottom); // flip y coordinate (PS vs Win)
...
//#if defined(DBCS)
    // we always define glyph bitmap as procedure
//    psputs(pdev, "CI 9 get /D exch def /DX -1 def\n");
//    psputs(pdev, "{DX 1 add dup /DX exch def D exch get}\n" );
//    psputs(pdev, "imagemask end}def\n");
    // create local storage for 'BuildGlyph' procedure.

//    psputs(pdev, "/BuildGlyph load 0 7 dict put\n");
//#else // DBCS
...
//#endif // DBCS
    ...
    // maximum # of glyphs per base font.
...
/*#ifdef DBG
	{
		...
		Quality =  pdev->dm.dmPublic.dmPrintQuality;
         
		scale = PSFXTOL(pdev->cgs.psfxScaleFactor);

		resol = PS_RESOLUTION;

	DBGMSG(DBG_LEVEL_WARNING, "Enter DownloadedAsOutline\n");
	if (pfo->flFontType & TRUETYPE_FONTTYPE) {
       DBGMSG(DBG_LEVEL_WARNING,"Font is TrueType\n");
    }
	DBGMSG1(DBG_LEVEL_WARNING,"psfxscal=%x \n", (int)scale);
	DBGMSG1(DBG_LEVEL_WARNING,"Quality=%d \n", (int)Quality);
	DBGMSG1(DBG_LEVEL_WARNING,"resol=%d \n", (int)resol);
	res = PSFXTOL(pdev->cgs.psfxScaleFactor) * pdev->dm.dmPublic.dmPrintQuality /
                    PS_RESOLUTION ;
	DBGMSG1(DBG_LEVEL_WARNING,"the res =%d \n", (int)res);
    if ( res >= 75) { 
        DBGMSG(DBG_LEVEL_WARNING,"another is True \n");
    }
	else {
        DBGMSG(DBG_LEVEL_WARNING,"another is False ");
    }
...
#ifdef  KKBUGFIX
#define TYPE1_EQUIVALENT(pfo, pdlf) \
	(!((pdlf)->flSimulate & RASTER_FONTTYPE) && \
	((pfo)->iTTUniq == (pdlf)->iTTUniq) && \
	((pfo)->iFace == (pdlf)->iFace) && \
	(((pfo)->flFontType & (FO_SIM_ITALIC /*| FO_SIM_BOLD */ )) == \
	((pdlf)->flSimulate & (FO_SIM_ITALIC /* | FO_SIM_BOLD*/ ))))

#define TYPE3_EQUIVALENT(pfo, pdlf) \
	(((pdlf)->flSimulate & RASTER_FONTTYPE) && \
	((pfo)->iUniq == (pdlf)->iUniq))


		if ( (boutline && TYPE1_EQUIVALENT(pfo, pDlFont)) ||
			 (!boutline && TYPE3_EQUIVALENT(pfo,pDlFont)) ) {
//	            DBGMSG1(DBG_LEVEL_WARNING,"The found DL font is No.%d",cDownloadedFonts);
...
//	DBGMSG(DBG_LEVEL_WARNING,"Cannot find the DL fonts\n");
...
// 1 - the function has pushed font onto dictionray stack
// 0 - the function has not pushed...
// -1 - too many fonts are downloaded, we should flush downloaded fonts and
//  start anew again.
    ...
    do {
        ...
        while (cGlyphs-- > 0)  {
         ...
         // Download the glyph definition if it has not yet been done.
			...
			else if (GlyphAlreadyDefined(pdev, pDLFont, index) < 0) {

#ifdef DBG
    DbgPrint( "PSCRIPT!AddCharsToFont: index = %d not defined yet\n",
        index );
#endif // DBG

                bCreateFont = FALSE;

                if ((iGlyph = FindDeviceGlyphIndex(pdev, pDLFont, index)) < 0) {

                    // need to create new base font
                    bCreateFont = TRUE;


                    // Too many base fonts are downloaded.  Let us flush downloaded
                    // fonts and try it again.
//DBGMSG1(DBG_LEVEL_WARNING, "Create a New Base Font, iGlyph=%x\n",iGlyph);

// if pDLFont->cBaseFont == 1, it is the first time to create base font, and Add1Font( ) 
// been called first, it can ensure at least downloading one base font. 
// but if pDLFont->cBaseFont > 1, we must check if the pdev->cDownloadedBaseFonts already
// reached the limit of DLFOnts.

// fix bug #54950, if the VM of printer is so little that it just contains only one
// downloadable font, if we dont't add pDLFont->cBaseFont==1 clause, system will go to
// a dead loop and hang the system.

                   if ( (pDLFont->cBaseFont > 1) &&
						 (pdev->cDownloadedBaseFonts == pdev->maxDLFonts) ) {

                        if (bpushed) {
                            StopAddingCharsToFont( pdev, boutline );
                        }
                        return -1;
                    }
// if pDLFont->CBaseFont ==1, it is a first time to create basefont for this DLfont,
// and before here, Add1Font should be called and the pdev->cDownloadedBaseFonts has been 
// added 1 already.
					if ( pDLFont->cBaseFont > 1)
                       pdev->cDownloadedBaseFonts++;
                    iGlyph *= -1;
...
//    DbgPrint( "AddCharsToFont:iGlyph=%x, cloadedBaseFont=%d CDownLoadedFont=%d pDLFBase=%d\n",
//        iGlyph, pdev->cDownloadedBaseFonts, pdev->cDownloadedFonts, pDLFont->cBaseFont );
#endif // DBG
                // save current base font, get new glyph ID and
                // base font ID.

                iBaseFontSave = iBaseFont;
                iBaseFont = iGlyph >> 8;
                iGlyph &= 0xff;

                 // switching the base font

                if (iBaseFontSave != ~0 &&
                        iBaseFont != iBaseFontSave) {
...
#ifdef DBG
    DbgPrint( "AddCharsToFont:pDLFont->cGlyphs = %d\n", pDLFont->cGlyphs );
#endif // DBG
                    if (pDLFont->cGlyphs > 255) {
                        psprintf(pdev,
                            "\n%%BeginFont:Base %s-%d\n",
                            pdev->cgs.szFont, iBaseFont);

                        if (bCreateFont) {
                            psprintf(pdev, "/%s-%d /%s DupFont\n",
                                pdev->cgs.szFont, iBaseFont, pdev->cgs.szFont);
                            bCreateFont = FALSE;
                        }
                    ...
                    if (boutline) {
                        ...
                        if (pDLFont->cGlyphs > 255) {
                            ...
                            eexecPrintf(pdev,
                                "/%s-%d findfont dup /Private get begin /CharStrings get begin\n",
                                 pdev->cgs.szFont, iBaseFont);
                        } else {

                            eexecPrintf(pdev,
                                "/%s findfont dup /Private get begin /CharStrings get begin\n",
                                pdev->cgs.szFont);
                        }
                    }
                ...
                (boutline ? DownloadType1Char : DownloadType3Char)
                    (pdev, pfo, &pgp->hg, pxo, iGlyph, FALSE);

                 // we already have marked the codepoints to be used
                 // during the call to FindDeviceGlyphIndex.
...
#ifdef DBG
	if (boutline) {
		DBGMSG(DBG_LEVEL_WARNING,"DownLoad as OutLine\n");
    }
    else {
		DBGMSG(DBG_LEVEL_WARNING,"DownLoad as Bitmap\n");
    }
#endif
...
#if defined(DBCS)
            // Allow more than 255 glyphs for DBCS fonts.
            // (We will download such fonts in multiple of subfonts.)

            if (!ISDBCSCHARSET(pifi))
...
//DBGMSG1(DBG_LEVEL_WARNING,"New PS Font=%s\n",szFaceName);
    ...
    if (calledFromDrvTextOut) {
// ++++ ***** CAUTION ****
// ++++ the dictionary clean up code here should be synched with the one below.
// ++++
		if (bnewfont) {

		// clean up the dictrionary here, since we have divided the font-
		// definition code and glyph downloading code.  this is for the
		// DBCS font convenienace, but affects SBCS font handling.
          if (boutline) {
...
// ++++
                ...
                // we want to update fontname information prior to calling AddCharsToFont(),
                // since we are refering the font names within this.
		if (bnewfont) 
		...
		bpushed = AddCharsToFont(pdev, pfo, pxo, pstro, pDLFont, boutline, FALSE);
        if ( bpushed < 0 ) {

			// threshold reached. now more additional base font can
			// be allocated.  we now flush downloaded fonts and fake
			// that it is a brand new font to be downloaded

			bnewfont = TRUE;
			goto ForceFlush;
		}

        // now font dictioanry has been completed.
        bnewfont = FALSE;
        // pdev->cgs.iBaseFont = ~0;
	}
...
//--------------------------------------------------------------------
// VOID FreeMiscDLFontInfo(pdev, pDLFont)
//
// History:
//  Aug-10-1996      By     Weibing Zhan (weibz)
//     merge for NT-FE 4.0
//   xx-Oct-1995    -by-    Takashi Matsuzawa (takashim)
//  Wrote it for NT-J.
// -------------------------------------------------------------------
VOID
FreeMiscDLFontInfo(
    ...
    DLFONT *pDLFont )
{
    ...
    for ( i = 0; i < MAX_BASE_FONT; i++ ) {
        if ( pDLFont->BaseFonts[ i ] != NULL ) {
            HEAPFREE( pdev->hheap,(VOID *)pDLFont->BaseFonts[ i ] );
            pDLFont->BaseFonts[ i ] = NULL;
        }
    }
...
//--------------------------------------------------------------------
// VOID AllocateNewBaseFont(pdev, pDLFont)
//
//  Aug-10-1996      By     Weibing Zhan (weibz)
//     merge for NT-FE 4.0
//   xx-Oct-1995    -by-    Takashi Matsuzawa (takashim)
    ...
    DLFONT *pDLFont )
{
    ...
    if ((iBase = pDLFont->cBaseFont) >= MAX_BASE_FONT - 1)
        return -1;

    // allocate new chunk of memory

    if ((pDLFont->BaseFonts[iBase] = (DLBASEFONT *)HEAPALLOC(
            pdev->hheap, sizeof (DLBASEFONT))) == NULL)
        return -1;

    // mark as unused.  note slot 0 of each base font is reserved for
    // .notdef character.

    memset(pDLFont->BaseFonts[iBase]->GlyphID, 0xff,
        sizeof (pDLFont->BaseFonts[iBase]->GlyphID));
    pDLFont->BaseFonts[iBase]->GlyphID[0] = 0;

    // initialize link list information

    if (pDLFont->cBaseFont > 0)
    {
        pDLFont->BaseFonts[(pDLFont->iLastLink >> 8)]
            ->RecentUseList[(pDLFont->iLastLink & 0xff)] = (iBase << 8) + 0;
    }

    for (iSlot = 0; iSlot < 255; iSlot++)
    {
        pDLFont->BaseFonts[iBase]->RecentUseList[iSlot]
            = ((iBase << 8) + iSlot + 1);
    }
    pDLFont->BaseFonts[iBase]->RecentUseList[iSlot] = 0;
    pDLFont->iLastLink = (WORD)((iBase << 8) + iSlot);
    pDLFont->cBaseFont++;
	return  1;
}

//--------------------------------------------------------------------
// LONG GlyphAlreadyDefined(pdev, pDLFont, index)
//
// Return:
//   -1   - Glyph not defined yet.
//   > 0  - Index to the glyph within device font file.
//
//  Aug-10-1996      By     Weibing Zhan (weibz)
//     merge for NT-FE 4.0
//   xx-Oct-1995    -by-    Takashi Matsuzawa (takashim)
...
{
    WORD wTmp, wTmpSave;
    LONG iLink, iLinkSave;


    if (pDLFont->cBaseFont == 0)
    {
        // to be allocated in FindDeviceGlyphIndex()
        return -1;
    }
    else
    {
        ...
        do {
            iLinkSave = iLink;
            iLink = pDLFont->BaseFonts[(iLink >> 8)]->RecentUseList[(iLink & 0xff)];
            wTmp = pDLFont->BaseFonts[(iLink >> 8)]->GlyphID[(iLink & 0xff)];

            if (wTmp == 0xffff || wTmp == wIndex)
                break;
    ...
    // update the link list if hit

    if (wTmp == wIndex)
    {
        if ((pDLFont->BaseFonts[(iLinkSave >> 8)]->RecentUseList[(iLinkSave & 0xff)]
            = pDLFont->BaseFonts[(iLink >> 8)]->RecentUseList[(iLink & 0xff)]) == 0)
                pDLFont->iLastLink = (WORD)iLinkSave;
        pDLFont->BaseFonts[(iLink >> 8)]->RecentUseList[(iLink & 0xff)]
            = pDLFont->BaseFonts[0]->RecentUseList[0];
        pDLFont->BaseFonts[0]->RecentUseList[0] = (WORD)iLink;
    }
...
//--------------------------------------------------------------------
// LONG FindDeviceGlyphIndex(pDLFont, index)
//
// Return:
//    < 0 - Empty slot not available.
//    > 0 - Index to the newly allocated slot within device font
//             file for this glyph.  (negated)
//
//  Aug-10-1996      By     Weibing Zhan (weibz)
//     merge for NT-FE 4.0
//   xx-Oct-1995    -by-    Takashi Matsuzawa (takashim)
...
LONG FindDeviceGlyphIndex(pdev, pDLFont, wIndex)
DEVDATA *pdev;
DLFONT *pDLFont;
DWORD wIndex;
{
    ...
    iLink = 0;
    iLinkSave = pDLFont->iLastLink;

//#ifdef DBG
//	DbgPrint("\n Under FindDeviceGlyphIndex\n");
//#endif
    if (pDLFont->cBaseFont == 0)
    {
        if (AllocateNewBaseFont(pdev, pDLFont) < 0)
            return -1;
    ...
    else
    {
        ...
        do {
            iLinkSave = iLink;
//#ifdef DBG
//			DbgPrint("iLink1=%x ", iLink); 
//#endif
            iLink = pDLFont->BaseFonts[(iLink >> 8)]->RecentUseList[(iLink & 0xff)];
//#ifdef DBG
//	DbgPrint("iLink2=%x ", iLink); 
//#endif
            wTmp = pDLFont->BaseFonts[(iLink >> 8)]->GlyphID[(iLink & 0xff)];

//#ifdef DBG
//			DbgPrint("wTmp=%d \x", wTmp); 
//#endif
            if (wTmp == 0xffff || wTmp == wIndex)
                break;
    ...
    // if glyph not found and no unused slot, allocate new base font.  note slot 0
    // is reserved.

//#ifdef DBG
//	DbgPrint("The result iLink=%x\n", iLink);
//#endif
    if (iLink == 0)
    {
        if ( AllocateNewBaseFont(pdev, pDLFont) < 0)
        ...
        iLink = (((pDLFont->cBaseFont - 1) << 8) + 1);
        wTmp = 0xffff;
        lRet *= -1;
    }

    // glyph not found, and unused slot exits

    if (wTmp == 0xffff)
    {
        pDLFont->BaseFonts[(iLink >> 8)]->GlyphID[(iLink & 0xff)] = (WORD)wIndex;
        wTmp = (WORD)wIndex;
    }
