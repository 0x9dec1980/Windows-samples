...
#ifdef DBCS
extern INT   psputfixToBuffer(PSTR,DWORD, ...);
extern LONG  GlyphAlreadyDefined(DEVDATA *,DLFONT *, DWORD);
#endif
...

{
	...
	DBGMSG(DBG_LEVEL_WARNING, "Start:");
    DbgPrint("FontOBJ->iUniq=%d iFace=%d iTTUniq=%x\n",
		      pfo->iUniq, pfo->iFace, pfo->iTTUniq);
    {
    ...
    // Get  font name from the IFIMETRICS structure.

		pifiTT = FONTOBJ_pifi(pfo);

		pwstrTT = (PWSTR)((BYTE *)pifiTT + pifiTT->dpwszFaceName);


		len = wcslen(pwstrTT);
		DbgPrint("\n the Font Face name is:\n");
		for (i=0; i<len; i++) {
			value = *(pwstrTT+i ); 
			if (value >= 0x80 )
			   DbgPrint(" %x ", value );
			else
               DbgPrint("%c", (char)value);
        }

		cGly = pstro->cGlyphs;
		DbgPrint("\n");
		DBGMSG1(DBG_LEVEL_WARNING, "CGly=%d ",cGly);
		for (i=0; i<cGly; i++)  {
			WORD   wrd;
		...
		DbgPrint("\n\n");
     	if ( pfo->flFontType & DEVICE_FONTTYPE ) {
			DBGMSG(DBG_LEVEL_WARNING, "This is Device Font\n");
        }
	    if ( pfo->flFontType & TRUETYPE_FONTTYPE ) {
		    DBGMSG(DBG_LEVEL_WARNING, "This is TrueType Font\n");
        }
    ...
    tdata.bSimBold = tdata.bDeviceFont && (pfo->flFontType & FO_SIM_BOLD);
#endif // DBCS
    tdata.doReencode = FALSE;

/* #ifdef DBG
	if (tdata.bDeviceFont) {
		DBGMSG(DBG_LEVEL_WARNING, "Device or SubStitutable Font\n");
    } else  {
		DBGMSG(DBG_LEVEL_WARNING, "Non Device and  NonSubsti Font\n");
    }
#endif  */
...
/*#ifdef DBG
	{
		...
		cGly = pstro->cGlyphs;
		DbgPrint("\n");
		DBGMSG1(DBG_LEVEL_WARNING, "After Select Font CGly=%d ",cGly);
		for (i=0; i<cGly; i++)  {
			WORD   wrd;
...
//---------------------------------------------------------------------------
// BOOL GetBaseFont(DEVDATA *pdev, DLFONT *pDLFont, GLYPHPOS *pgp,
//                 BYTE *pBaseFont, BYTE *piGlyph );
//
// Get the base font ID for the specified glyph.  Since conventional (Type1 or
// type3) PostScript fonts cannot hold more than 256 glyphs in it, we maintain
// a downloaded TrueType fonts in multiple base (sub) fonts.
//
// History:
//   7-Aug-1996    Merge to NT 4.0 and change the DBG Macro by Weibz
//   1-Jul-1995    -by-    Takashi Matsuzawa (takashim)
// Written.
//---------------------------------------------------------------------------
    ...
    BYTE *piGlyph )
{
    ...
    iGlyph = GlyphAlreadyDefined(pdev, pDLFont, iIndex);
    if (iGlyph < 0) {

       DBGMSG1(DBG_LEVEL_ERROR,
		   "GetBaseFont: Glyph not found in donloaded font (%d)\n", iGlyph);
        return FALSE;
    }

    *pBaseFont =(BYTE)(iGlyph >> 8);
    *piGlyph = (BYTE)(iGlyph & 0xff);
    return TRUE;
}

//---------------------------------------------------------------------------
// VOID SwitchBaseFont(DEVDATA *pdev, DLFONT *pDLFont, WORD iBaseFont );
//
// Switch to the specified base font.
//
// History:
//   7-Aug-1996    Merge to NT 4.0 and change the PsPrintf by Weibz
//   1-Jul-1995    -by-    Takashi Matsuzawa (takashim)
// Written.
//---------------------------------------------------------------------------
...
//	DBGMSG(DBG_LEVEL_WARNING, "Under SwitchBaseFont\n");

    if (pDLFont->cGlyphs > 255) {
         SPRINTF(szSuffix, "-%d", iBaseFont);
    } else {
         szSuffix[0] = '\0';
    }

    psprintf(pdev, pdev->cgs.szSetFont, szSuffix);
    pdev->cgs.iBaseFont = iBaseFont;
}
...
//#ifdef KKBUGFIX  // for Bold Simulate printing
//		psputs(pdev, "gsave \n0 setlinewidth\n");
//#endif
        ps_newpath(pdev);
...

{
	...
	// if it is  DBCS GDI fonts, we will postpone outputing.     
	if (pdev->cgs.bDBCSGDIFont == FALSE)
#endif  //DBCS
	psputs(pdev, "[");
...
#ifdef DBCS
// if it is a DBCS GDI font, we will postpone the outputing.
		if ( pdev->cgs.bDBCSGDIFont ) {
			
			INT    cCount;

			pStr = pdev->cgs.szSetFont;
			*pStr++ = '[';
			cCount = psputfixToBuffer(pStr,6,fontsize, 0, 0, -fontsize, 0, 0);
			pStr = pdev->cgs.szSetFont + cCount + 1;
        } else
    ...
    } else {   // it is Device Font or Outline font
...
// if it is a DBCS GDI font, we will postpone the outputing.
		if ( pdev->cgs.bDBCSGDIFont ) {
			...
			pStr = pdev->cgs.szSetFont;
			*pStr++ = '[';
			cCount = psputfixToBuffer(pStr,6, 
	...
	if ( pdev->cgs.bDBCSGDIFont) {
		SPRINTF(pStr, "]/%s%s",
			(bReencoded || bSimVert) ? "_" : "", pdev->cgs.szFont);
		pStr += strlen(pStr);
		if (bSimVert) {
			 strcpy(pStr,"%s-V MF\n");
			 pStr +=8;
...
#ifdef DBCS   
	// first we just set the flag as FALSE, and later if it is really a GDI DBCS font, it 
	// will be set TRUE again.
    pdev->cgs.bDBCSGDIFont =  FALSE;
#endif

    if (pdata->bDeviceFont) {

//    DBGMSG(DBG_LEVEL_WARNING,"Handle DeviceFont\n");
...
#if defined(DBCS) // FE font
        if (! currentFontInSync || pdev->cgs.pDevFont == NULL) {
#else // DBCS
        if (! currentFontInSync) {
            ...
            pdev->cgs.pDevFont = NULL;
            if (iFace < pdev->cDeviceFonts) {
                pdev->cgs.pDevFont = (PDEVFONT)LISTOBJ_FindIndexed((PLISTOBJ)
                            pdev->hppd->pFontList, iFace);
                if (pdev->cgs.pDevFont != NULL) {
                    pFontName = pdev->cgs.pDevFont->pName;
                }
                else {
                    DBGMSG1(DBG_LEVEL_ERROR,
                        "Could not find DEVFONT for typeface %ld", iFace);
                }
            }
            else
                pFontName = (PSTR) pntfm + pntfm->ntfmsz.loszFontName;
#else // DBCS
...
#ifdef DBG
		DbgPrint("wCodePage=%d\n", pdev->cgs.pDevFont->wCodePage);
#endif

            if (IsDBCSCodePage(pdev->cgs.pDevFont->wCodePage)) {

                // We do not Latin-reencode FE fonts

                pdata->doReencode = FALSE;

                // We do not reencode, but need to make modifications
                // to the font dictionray if it is a vertical writing
                // font.  Send out the "VF" command, and mark that we
                // have reencoded it.

                if (pdev->cgs.pDevFont->wVertical != FEVERT_NONE) {
                    if (!QueryFontRemap(pdev, pfo->iUniq)) {
                        psprintf(pdev, "/_%s%s /%s VF\n",
                            pdev->cgs.szFont,
                            (pdev->cgs.pDevFont->wVertical
                                    == FEVERT_SIMULATED ? "-V" : ""),
                            pdev->cgs.szFont);
                        SetFontRemap(pdev, pfo->iUniq);
                    }
                }
            }
#endif // DBCS
...
//    DBGMSG(DBG_LEVEL_WARNING,"Handle  GDI Font and Non-substiFonts\n");
...
// if it is a GDI DBCS font, we has downloaded it as multi Base Fonts,
//		so mark it in pdev->cgs
#ifdef DBCS  

		pdev->cgs.pDevFont = NULL;
		if (FONTOBJ_cGetAllGlyphHandles(pfo, NULL) > 255)
			pdev->cgs.bDBCSGDIFont = TRUE;
		else
            pdev->cgs.bDBCSGDIFont =  FALSE;
#endif
...
#if defined(DBCS)
        WORD wVertical = (pdev->cgs.pDevFont != NULL)
                ? pdev->cgs.pDevFont->wVertical : FEVERT_NONE;

#ifdef DBG

		switch (wVertical) {
		case  FEVERT_NONE:     DbgPrint("wVertical=FEVERT_NONE\n"); break;
		case FEVERT_SIMULATED : DbgPrint("wVertical=FEVERT_SIMULATED\n"); break;
		default :  DbgPrint("wVertical=FEVERT Real\n"); break;
        }
#endif

        PsMakeFont(pdev,
            pdata->bDeviceFont && (QueryFontRemap(pdev, pfo->iUniq) || wVertical != FEVERT_NONE),
            pdata->bSimItalic,
            ! pdata->bDeviceFont && ! boutline,
#if defined(FE_BOLD)
            pdata->bSimBold,
#endif // FE_BOLD
            (wVertical == FEVERT_SIMULATED));
#else // DBCS
        PsMakeFont(pdev,
...
#ifdef  DBCS
			if (pdev->cgs.bDBCSGDIFont) // if GDI DBCS Font, every glyph has two bytes value
			    charStr ++;
#endif
...
#ifdef DBCS
			if (pdev->cgs.bDBCSGDIFont) {
                    BYTE   iCurrentBaseFont;

					iCurrentBaseFont = *charStr++;
                    if (iCurrentBaseFont != pdev->cgs.iBaseFont)  // we need to change to another
						                                // base font.
                    {
						psputs(pdev, ")t\n");
                        SwitchBaseFont(pdev, pDLFont, iCurrentBaseFont);
						psputs(pdev, "(");
                    }
...

{
    ...
    ULONG       cBytes;
    PSTR        pChunk;
    ULONG       cBytesChunk, cCharsChunk;
    BOOL        bVert;
    ULONG       k;
	BYTE        iBaseFont, iGlyph;        
#endif // DBCS
...
#if defined(DBCS) // FE font
    // Allocate possible maximum (count DBCS glyphs) size for buffer.

    if ((pch = charStr = MEMALLOC(cGlyphs * sizeof(CHAR) * 4)) == NULL) {
        DBGERRMSG("MEMALLOC");
        return FALSE;
    }
    charStr2 = charStr + cGlyphs * 2;
#else // DBCS
...
#ifdef DBCS
		// For DBCS downloaded GDI font, because one downloaded font contains many 
		// BaseFonts, and every BaseFont contains up to 256 glyphs, so to encode
		// the specific char, we must convert a glyph handle to two bytes, First Byte
		// is number of BaseFont and second byte is index inside this basefont.
		// Weibing Zhan,  Aug-10-1996

		if ( pdev->cgs.bDBCSGDIFont )
           for (count=0, pgp =pGlyphPos; count < cGlyphs; count++, pgp++) {
              if (GetBaseFont(pdev, pDLFont, pgp, &iBaseFont, &iGlyph) == FALSE) {
			  	  *pch++ = NUL;
				  *pch++ = NUL;
				  bCharBitmap = TRUE;
               } else  {
           ...
           for (count=0, pgp =pGlyphPos; count < cGlyphs; count++, pgp++) {
              if (GetBaseFont(pdev, pDLFont, pgp, &iBaseFont, &iGlyph) == FALSE) {
				  ...
				  *pch++ = iGlyph;   // we dont take care of iBaseFont, because
				                     // all these chars are contained in one font
               }
...
DbgPrint("pdev->cgs.pDevFont->wCodePage=%d\n", pdev->cgs.pDevFont->wCodePage);
		
#endif
		
		if (IsDBCSCodePage(pdev->cgs.pDevFont->wCodePage)) {

           UNICODETOMULTIBYTE(pch, cGlyphs*sizeof(CHAR)*2, &cBytes,
                           pwstr, cGlyphs*sizeof(WCHAR));
        }  else  { 
			// for SBCS fonts, we just use 1252 as code page to convert 
			// Unicode to priter device char code.


			cBytes = WIDECHARTOMULTIBYTE(1252, pwstr, cGlyphs*sizeof(WCHAR), 
				                pch, cGlyphs*sizeof(CHAR)); 
                ...
                (pdev->pPrinterData->dwFlags & PSDEV_SLOW_FONTSUBST)))
    {
        ...
        if (FALSE && Level2Device(pdev->hppd)) {
            ...
            if (pdata->bDeviceFont) {
                ...
                while (cBytes > 0) {
                    ...
                    bVert = ScanFullWidthGlyphs(pdev->cgs.pDevFont,
                            pChunk, cBytes, &cBytesChunk, &cCharsChunk);

                    // Reuse this as temporal variable
                    ...
                    psputs(pdev, "(");
                    k = ConvertToDeviceCode(pdev->cgs.pDevFont, charStr2,
                           pChunk, cBytesChunk);
                    bResult = RemapDeviceChars(pdev, k, charStr2);
                    psputs(pdev, ")\n[");
...
#if defined(DBCS) // FE font
        if (pdata->bDeviceFont && (pdata->iFace <= pdev->cDeviceFonts) ) {
            ...
            while (cBytes > 0) {
                ...
                for (count = 1, pch = pChunk;
                    count <= cBytesChunk && bResult;
                    count++, pGlyphPos++)
                {
                    psputs(pdev, "(");

                    // Need to take care of DBCS glyphs
                    if ((k = ConvertToDeviceCode(pdev->cgs.pDevFont,
                            charStr2, pch, 1)) == 0) {
                        k = ConvertToDeviceCode(pdev->cgs.pDevFont,
                                charStr2, pch, 2);
                        pch++;
                        count++;
                    }
                    bResult = RemapDeviceChars(pdev, k, charStr2);
                    pch++;
                    psprintf(pdev, ")%d %d %s%c%c",
                        pGlyphPos->ptl.x, pGlyphPos->ptl.y,
                        (pdata->bSimBold ? "MSB" : "MS"),
						(bVert ? 'V':' '),
                        ((count & 15) && count != cBytesChunk) ? ' ' : '\n');
                }
                pChunk += cBytesChunk;
                cBytes -= cBytesChunk;
            }
        ...
        // The optimal case: We're using a device font or downloaded font and don't
        ...
        if (pdata->bDeviceFont && (pdata->iFace <= pdev->cDeviceFonts)) {
            ...
            while (cBytes > 0) {
				...
				bVert = ScanFullWidthGlyphs(pdev->cgs.pDevFont,
                    pChunk, cBytes, &cBytesChunk, &cCharsChunk);
                psputs(pdev, "(");
                k = ConvertToDeviceCode(pdev->cgs.pDevFont,charStr2, pChunk, cBytesChunk);
                bResult = RemapDeviceChars(pdev, k, charStr2);
                psputs(pdev, ")");
                if (!bMoved) {
                    psprintf(pdev, "%d %d %s%c\n",
                        pGlyphPos->ptl.x, pGlyphPos->ptl.y, 
                        (pdata->bSimBold ? "MSB" : "MS"),
						(bVert ? 'V':' ')
					);
                ...
                pGlyphPos += cCharsChunk;
                pChunk += cBytesChunk;
                cBytes -= cBytesChunk;
            }
        }
        else if (pdev->cgs.bDBCSGDIFont) {
					char		pstr[256];
					int         iNum=0;	
//					int		    xMove, yMove;
                    BYTE		iCurrentBaseFont;
			...
			// Move to the first position
		 	    psprintf(pdev, "%d %d M", pGlyphPos->ptl.x, pGlyphPos->ptl.y);
				for (count =1, pch=charStr;
				     count <=cGlyphs && bResult;
					 count++) 

			    {
                    ...
                    { if (iNum > 0) {
                         ...
                         psputs(pdev, "(");
						 bResult = RemapDeviceChars(pdev,iNum, pstr);
                         if (((pfo->flFontType & FO_SIM_BOLD) != 0) &&
							 DownloadedAsOutline(pdev, pfo) )
       						 psprintf(pdev, ")SB%c",
                               ((count & 15) && count != cGlyphs) ? ' ' : '\n');
...
//						 xMove = iNum * (pGlyphPos[1].ptl.x - pGlyphPos[0].ptl.x);
//						 yMove = iNum * (pGlyphPos[1].ptl.y - pGlyphPos[0].ptl.y);
//	                     psprintf(pdev, "%d %d rm", xMove, yMove);
						 iNum = 0;
					  }
                      SwitchBaseFont(pdev, pDLFont, iCurrentBaseFont);
                    }
...
#if defined(DBCS) // FE font
        // Allow font substitution for DBCS strings.
        // On DBCS environment, ACP (0) is a DBCS codepage.

        // More precisely/ideally, you should check if the device
        // font encoding matches the current string's codepage.

        if (IsDBCSCodePage(GetACP()))
            return TRUE;
#endif // DBCS
