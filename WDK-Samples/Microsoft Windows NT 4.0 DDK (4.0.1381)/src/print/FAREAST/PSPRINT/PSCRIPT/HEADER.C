...
#if defined(DBCS) // non-ASCII document names
BOOL RemapDeviceChars(PDEVDATA, DWORD, PCHAR);
#endif // DBCS
...

{
...
#if defined(DBCS) // non-ASCII document name
        psputs(pdev, "%%%%Title: ");
        RemapDeviceChars(pdev, strlen(buf), buf);
        psputs(pdev, "\n");
#else // DBCS
    ...
    if (hppd->pJobPatchFileList != NULL ) {

		PJOBPATCHFILE   pJobPatch = hppd->pJobPatchFileList;

		while  (pJobPatch != NULL )  {

			DscBeginFeature(pdev, "JobPatchFile ");
			psputs(pdev, pJobPatch->pName);
			psputs(pdev, "\n");
			psputs(pdev, pJobPatch->pInvocation);
			DscEndFeature(pdev);			
			pJobPatch=(PJOBPATCHFILE)(pJobPatch->pNext);
        }
    }
	...
	// merge new code for negative print out 
	...
	psputs(pdev, PROCSETNAME);
	psprintf(pdev, " /blw %d 600.0 div put\n", pdev->dm.dmPublic.dmPrintQuality);
...

{
    ...
    cOptions = pdev->dm.dmPrivate.wOptionCount;
    pOptions = pdev->dm.dmPrivate.options;
    ...
    for (index=0; index < cOptions; index++, pOptions++) {

        if (*pOptions != OPTION_INDEX_ANY) {

            pOrderDep = PpdFindOrderDep(pdev->hppd, index, *pOptions);

            if (pOrderDep != NULL &&
                (pOrderDep->section & (ODS_PROLOG | ODS_EXITSERVER)))
            {
                // Features in Prolog/ExitServer section are not handled.

                DBGMSG(DBG_LEVEL_ERROR, "Prolog/ExitServer features ignored!\n");
