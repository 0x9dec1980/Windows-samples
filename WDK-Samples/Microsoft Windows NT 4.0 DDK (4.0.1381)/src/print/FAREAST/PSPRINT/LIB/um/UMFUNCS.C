...

{
    ...
    hRes = FindResource(hinst, (LPCTSTR)id, (LPCTSTR)WINRT_FONTMETRIC);

    if (hRes == NULL) {

        DBGMSG(DBG_LEVEL_ERROR, "FindResource failed.\n");
    }
    else {
        hResData = LoadResource(hinst, hRes);
        pntfm = (PNTFM) LockResource(hResData);
    }
