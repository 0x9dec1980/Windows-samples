...
ULONG cjGetFamilyAliases(IFIMETRICS *, PSTR, DWORD);
#else // DBCS
...
#if defined(DBCS) // FE font
#define FontIsBold(pPfm)  (READ_WORD(&(pPfm)[OFF_Weight]) > FW_NORMAL)
#define FontIsDBCS(pPfm)  (IsDBCSCharSet((pPfm)[OFF_CharSet]))
#endif // DBCS
...
#if defined(DBCS) // FE font
    charset = pjPFM[OFF_CharSet];
    if (charset == SYMBOL_CHARSET || charset == DEFAULT_CHARSET)
        charset = ANSI_CHARSET;

    TranslateCharsetInfo((DWORD *)charset, &ci,
            TCI_SRCCHARSET);
#endif // DBCS
...
#if defined(DBCS) // FE font
        MULTIBYTETOWIDECHAR(ci.ciACP, awcKern, (2 * sizeof(WCHAR)),
            pjKern, 2);
#else // DBCS
...
#if defined(DBCS)
        // if it is a DBCS font, width of single byte characters should
        //  be the half of max character (double byte character) width.
        if (FontIsDBCS(pjPFM))
            usWidth /= 2;
#endif // DBCS
...
#define C_CHAR 256
    ...
    // This information should be dinamically retrieved, but I do
    // not find appropriate system database for this.

    switch (pifi->jWinCharSet) {
    case CHINESEBIG5_CHARSET:
        pifi->wcFirstChar = U_CHINESEBIG5_FIRST_CHAR;
        pifi->wcLastChar = U_CHINESEBIG5_LAST_CHAR;
        return;
    case GB2312_CHARSET:
        pifi->wcFirstChar = U_GB2312_FIRST_CHAR;
        pifi->wcLastChar = U_GB2312_LAST_CHAR;
        return;
    case HANGEUL_CHARSET:
        pifi->wcFirstChar = U_HANGEUL_FIRST_CHAR;
        pifi->wcLastChar = U_HANGEUL_LAST_CHAR;
        return;
    case SHIFTJIS_CHARSET:
        pifi->wcFirstChar = U_SHIFTJIS_FIRST_CHAR;
        pifi->wcLastChar = U_SHIFTJIS_LAST_CHAR;
        return;
    }
...
#if defined(DBCS) // FE font
    // For others, default to Windows ANSI.

    MULTIBYTETOWIDECHAR(1252, awc, (C_CHAR * sizeof(WCHAR)),
             ach, C_CHAR);
#else // DBCS
...
#if defined(DBCS) // FE font
ULONG cjGetFamilyAliases(pifi, pstr, charset)
IFIMETRICS *pifi;
PSTR        pstr;
DWORD       charset;
#else // DBCS
...
#endif // DBCS
{
    ...
    // set the pifi->flInfo flag.
        ...
        {
#if defined(DBCS) // FE font
            cb = CopyStr2WideChar(ci.ciACP, pwstr, *pTable, 0);
            if (pifi) {
                CopyStr2WideChar(ci.ciACP, pwstr, *pTable, cb);
            }
            cb++;
#else // DBCS
            if (pifi) {
...
#if defined(DBCS) // FE font
        cbRet = CopyStr2WideChar(ci.ciACP, pwstr, pstr, 0);
        if (pifi) {
            CopyStr2WideChar(ci.ciACP, pwstr, pstr, cbRet);

        }
        cbRet = (cbRet + 1) * sizeof(WCHAR);
#else // DBCS
        if (pifi) {
...
// which is then rounded up to a DWORD boundary for faster access
    ...
    cjIFI = ALIGN4(sizeof(IFIMETRICS))                   +
            ...
            ALIGN4(cjGetFamilyAliases(NULL,pszFaceName,pjPFM[OFF_CharSet])) ;
#else // DBCS
    cjIFI = ALIGN4(sizeof(IFIMETRICS))                   +
            (bSoft ? offsetof(IFIEXTRA,dpFontSig) : 0)   +
...
#if defined(DBCS) // FE font
// We simulate italic, bold, bolditalic for any DBCS typeface.
// For other fonts, we do not do redundant simulation.

    if (FontIsDBCS(pjPFM)|| !FontIsItalic(pjPFM) && !FontIsBold(pjPFM))
        cjIFI += ALIGN4(sizeof(FONTSIM)) + ALIGN4(sizeof(FONTDIFF) * 3);
    else if (!FontIsItalic(pjPFM) && FontIsBold(pjPFM) ||
            FontIsItalic(pjPFM) && !FontIsBold(pjPFM))
...
#if defined(DBCS) // FE font
    cjFamilyAliases = cjGetFamilyAliases(pifi, pszFaceName, pjPFM[OFF_CharSet]);
#else // DBCS
...
#if defined(DBCS)
    // now correct the above assumptions for DBCS device fonts.

    if (FontIsDBCS(pjPFM))
    {
        // note FM_INFO_CONSTANT_WIDTH should not be set for DBCS font because
        // we have at least two widths within a font: halfwidth and fullwidth.
        // See also DrvQuryFontData() code.
...
#if defined(DBCS)
    switch(pjPFM[OFF_CharSet]) {
    case CHINESEBIG5_CHARSET:
        pifi->chBreakChar = A_CHINESEBIG5_BREAK_CHAR;
        pifi->wcBreakChar = U_CHINESEBIG5_BREAK_CHAR;
        pifi->chDefaultChar = A_CHINESEBIG5_DEFAULT_CHAR;
        pifi->wcDefaultChar = U_CHINESEBIG5_DEFAULT_CHAR;
        break;
    case GB2312_CHARSET:
        pifi->chBreakChar = A_GB2312_BREAK_CHAR;
        pifi->wcBreakChar = U_GB2312_BREAK_CHAR;
        pifi->chDefaultChar = A_GB2312_DEFAULT_CHAR;
        pifi->wcDefaultChar = U_GB2312_DEFAULT_CHAR;
        break;
    case HANGEUL_CHARSET:
        pifi->chBreakChar = A_HANGEUL_BREAK_CHAR;
        pifi->wcBreakChar = U_HANGEUL_BREAK_CHAR;
        pifi->chDefaultChar = A_HANGEUL_DEFAULT_CHAR;
        pifi->wcDefaultChar = U_HANGEUL_DEFAULT_CHAR;
        break;
    case SHIFTJIS_CHARSET:
        pifi->chBreakChar = A_SHIFTJIS_BREAK_CHAR;
        pifi->wcBreakChar = U_SHIFTJIS_BREAK_CHAR;
        pifi->chDefaultChar = A_SHIFTJIS_DEFAULT_CHAR;
        pifi->wcDefaultChar = U_SHIFTJIS_DEFAULT_CHAR;
        break;
    default:
        // Default to Windows ANSI.

        MULTIBYTETOWIDECHAR(1252, &pifi->wcDefaultChar,
                sizeof(WCHAR), &pifi->chDefaultChar, 1);
        MULTIBYTETOWIDECHAR(1252, &pifi->wcBreakChar,
                sizeof(WCHAR), &pifi->chBreakChar, 1);
    }
#else // DBCS
...
#if defined(DBCS)
    // Set appropriate font simulation information.

    bBoldFs = (FontIsDBCS(pjPFM) || !FontIsBold(pjPFM));
    bItalicFs = (FontIsDBCS(pjPFM) || !FontIsItalic(pjPFM));

    if (bBoldFs || bItalicFs) {
        ...
        if (bBoldFs) {
            pFontSim->dpBold = cjIFI - loFontSim;
        ...
        if (bItalicFs) {
            pFontSim->dpItalic = cjIFI - loFontSim;
            cjIFI += ALIGN4(sizeof(FONTDIFF));
        }
        else {
            pFontSim->dpItalic = 0;
        ...
        if (bBoldFs && bItalicFs) {
            pFontSim->dpBoldItalic = cjIFI - loFontSim;
            cjIFI += ALIGN4(sizeof(FONTDIFF));
        }
        else {
        ...
        if (bBoldFs) {
            pFontDiff = (FONTDIFF *)((BYTE *)pFontSim + pFontSim->dpBold);
            *pFontDiff = FontDiff;
        ...
        if (bItalicFs) {
            pFontDiff = (FONTDIFF *)((BYTE *)pFontSim + pFontSim->dpItalic);
            *pFontDiff = FontDiff;
        ...
        if (bBoldFs && bItalicFs) {
            pFontDiff = (FONTDIFF *)((BYTE *)pFontSim + pFontSim->dpBoldItalic);
            *pFontDiff = FontDiff;
            pFontDiff->bWeight = PAN_WEIGHT_BOLD;
