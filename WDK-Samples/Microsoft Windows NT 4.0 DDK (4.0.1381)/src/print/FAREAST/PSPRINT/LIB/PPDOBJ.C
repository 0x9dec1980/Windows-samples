...
Arguments:
    ...
    ppListObj - pointer to a linked list.
    pItem - pointer to the item to be deleted
...
[Note:]
    ...
    The first parameter is declared as PLISTOBJ* because
    we need to modify the pointer to linked list itself
    in case it became empty.

    The caller is responsible to destroy the storage for
    the item deleted.
...

{
    ...
    dwHash = HashKeyword(pItem->pName);
    ...
    while ((pNext != NULL) && 
           (dwHash != pNext->dwHash ||
            strcmp(pItem->pName, pNext->pName) != EQUAL_STRING))
    {
        pLast = pNext;
        pNext = pLast->pNext;
    }

    // Remove the item from the list if it was found.
    // Modify address of the passed list if it was the first entry
    // in the list.

    if (pNext != NULL) {
        if (pNext == *ppListObj)
            *ppListObj = pNext->pNext;
        else
...
{
#if (DBCS) // FE font
    DBGPRINT("Font %s, encoding = %d, charset = %d, wIndex = %d,"
             " wVertical = %d\n",
             pFont->pName, pFont->wEncoding, pFont->wCharSet,
             pFont->wIndex, pFont->wVertical);
#else // DBCS
