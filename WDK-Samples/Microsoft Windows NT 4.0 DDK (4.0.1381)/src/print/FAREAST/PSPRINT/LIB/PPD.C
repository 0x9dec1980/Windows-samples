...
#ifdef DBCS  // Output Glyph Prefer Option

#ifdef DBG

VOID
PpdFont_Downloading(HPPD hppd)
{
	
	PDEVFONT    pCurrentFont;

	// downloading Device Font List 

    pCurrentFont = hppd->pFontList;

	DbgPrint("\n Downloading Device Font List: %s\n", hppd->pNickName);
    ...
    while  ( pCurrentFont != NULL )  {
		...
		wEncoding = pCurrentFont->wEncoding;
        wCharSet  = pCurrentFont->wCharSet;
        wIndex    = pCurrentFont->wIndex;
        wCodePage = pCurrentFont->wCodePage;
        wVertical = pCurrentFont->wVertical;
		         
		DbgPrint("Name:%s, Xlation=%s\n",pCurrentFont->pName,pCurrentFont->pXlation);  
		if  (wEncoding == FONTENC_RKSJ)
			DbgPrint("FONTENC_RKSJ  ");
		else
			if ( wEncoding == FONTENC_STANDARD)
			    DbgPrint("FONTENC_STAD  ");
            DbgPrint("FONTENC_OTHR  ");
		switch (wCharSet)  {
		case CHARSET_90MS :  DbgPrint("CHARSET_90MS  ");  break;
		case CHARSET_83PV :  DbgPrint("CHARSET_83PV  ");  break;
		case CHARSET_EXT  :  DbgPrint("CHARSET_EXT   ");  break;
		default :            DbgPrint("CHARSET_OTHR  ");  break;
        }
		DbgPrint("wIndex=%d wCP=%d ",wIndex, wCodePage);
   		switch (wVertical) {
		case FEVERT_NONE      : DbgPrint("FEVERT_NONE\n\n"); break;
		case FEVERT_SIMULATED : DbgPrint("FEVERT_SIMU\n\n"); break;
		case FEVERT_VERTICAL  : DbgPrint("FEVERT_VERT\n\n"); break;
		default :               DbgPrint("FEVERT_OTHR\n\n"); break;
        }

  	    pCurrentFont = pCurrentFont->pNext;
    }
...
BOOL
bContainJPNFonts( HPPD   hppd )  
{
	PDEVFONT    pCurrentFont;
	BOOL		bContainJPN=FALSE;
	...
	while ( pCurrentFont != NULL ) {
	     ...
	     if ( pCurrentFont->wEncoding == FONTENC_RKSJ )  {
			 bContainJPN = TRUE;
			 break;
         }
		 pCurrentFont = pCurrentFont->pNext;
...
VOID  
PpdValidate_On_Default(
	   HPPD		hppd)  
{
	PDEVFONT    pFontList;
	PDEVFONT    pLastFont, pCurrentFont, pStartFont, pEndFont, pEndNextFont;
	PDEVFONT    pTmpFont;
    

	
	if ( bContainJPNFonts(hppd) == FALSE )  return;

	pFontList = hppd->pFontList;
    pLastFont = pCurrentFont = pFontList;

	while ( pCurrentFont != NULL ) {
		pStartFont = pEndFont = pCurrentFont;
		pEndNextFont = pEndFont->pNext;
		while ( (pEndNextFont != NULL )  &&
			    (pEndNextFont->wIndex == pStartFont->wIndex) ) {
                   pEndFont = pEndNextFont;
				   pEndNextFont= pEndFont->pNext;
        }
        if (pStartFont == pEndFont ) {
			pLastFont = pCurrentFont;
			pCurrentFont = pEndFont->pNext;
			continue;
        }
        pTmpFont = pStartFont;
		while (pTmpFont != pEndFont ) {
			if ( pTmpFont->wCharSet ==  CHARSET_90MS )  break;
			pTmpFont = pTmpFont->pNext;
        }

		if ( pTmpFont->wCharSet != CHARSET_90MS ) {
			pTmpFont = pStartFont;
            while (pTmpFont != pEndFont) {
			   if ( pTmpFont->wCharSet ==  CHARSET_83PV )  break;
			   pTmpFont = pTmpFont->pNext;
            }
        }
		pLastFont->pNext = pTmpFont;
		pTmpFont->pNext = pEndFont->pNext;
		pLastFont = pTmpFont;
		pCurrentFont=pTmpFont->pNext;

    }
			

    hppd->wCurOutGlyPreferOpt = PPD_OGPO_DEFAULT;

}


VOID  
PpdValidate_On_JIS78(
	   HPPD		hppd)  
{
	PDEVFONT    pFontList;
	PDEVFONT    pLastFont, pCurrentFont, pStartFont, pEndFont, pEndNextFont;
	...
	while ( pCurrentFont != NULL ) {
        ...
        pTmpFont = pStartFont;
		while (pTmpFont != pEndFont ) {
			if ( pTmpFont->wCharSet ==  CHARSET_EXT )  break;
			pTmpFont = pTmpFont->pNext;
        }
		pLastFont->pNext = pTmpFont;
		pTmpFont->pNext = pEndFont->pNext;
    ...
    hppd->wCurOutGlyPreferOpt = PPD_OGPO_JIS78;
			
}


VOID  
PpdValidate_On_FontPreferOption(
	HPPD   hppd,
	WORD   wCurOutGlyPreferOpt )
{

	switch ( wCurOutGlyPreferOpt ) {
	case  OGPO_DEFAULT:
		  PpdValidate_On_Default(hppd);
		  break;
    case  OGPO_JIS78:
		  PpdValidate_On_JIS78(hppd);
		  break;
    }
    ...
    if (hppd != NULL) {
        ...
        if (hppd->wParserVersion != PPD_PARSER_VERSION) {
                        ...
                        StripDirPrefixW(hppd->pwstrFilename)) != EQUAL_STRING)
            {
				...
				PpdValidate_On_FontPreferOption(hppd, wCurOutGlyPreferOpt );
#endif
...
#if defined(DBCS) // FE font
    if ((ppdobj = PPDOBJ_Create(pwstrFilename, hmodule)) == NULL) {
#else // DBCS
...

{
    ...
    curOffset += PackPstr(pbuf, curOffset, &pJobPatchFile->pXlation);
    curOffset += PackPstr(pbuf, curOffset, &pJobPatchFile->pInvocation);
...
#ifdef KKBUGFIX  // JobPatchFile support
    dwSize += PackListObj(
                    pbuf, dwSize, (PLISTOBJ *) &hppd->pJobPatchFileList,
                    sizeof(JOBPATCHFILE), (PACKPROC) PackJobPatchFile);
...
{
    OffsetToPointer(hppd, &pJobPatchFile->pXlation);
    OffsetToPointer(hppd, &pJobPatchFile->pInvocation);
...

{
...
#ifdef KKBUGFIX  // JobPatchFile support
    OffsetToPointer(hppd, &hppd->pJobPatchFileList);
#endif
    ...
    UnpackListObj(
        hppd,
        (PLISTOBJ) hppd->pJobPatchFileList,
        (UNPACKPROC) UnpackJobPatchFile);
