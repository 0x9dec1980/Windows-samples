...
TT_FONT_MAPPING TTFontTable[] = {
    ...
    // For latin fonts use typeface name not font name, to aligh with
    // DBCS fonts to whcih we must use typeface names
    ...
    // FE fonts (JPN)
    IDS_MS_GOTHIC,
            // HeiseiMin-W3-RKSJ-H (Heisei Kaku Gothic W5)
            L"\x5e73\x6210\x89d2\x30b4\x30b7\x30c3\x30af\x0057\x0035",
    IDS_MS_GOTHIC_VERT,
            // HeiseiMin-W3-RKSJ-V (@ Heisei Kaku Gothic W5)
            L"\x0040\x5e73\x6210\x89d2\x30b4\x30b7\x30c3\x30af\x0057\x0035",
    IDS_MS_MINCHO,
            // HeiseiKakuGo-W5-RKSJ-H (Heisei Mincho W3)
            L"\x5e73\x6210\x660e\x671d\x0057\x0033",
    IDS_MS_MINCHO_VERT,
            // HeiseiKakuGo-W5-RKSJ-V (@ Heisei Mincho W3)
            L"\x0040\x5e73\x6210\x660e\x671d\x0057\x0033",
    IDS_MS_GOTHIC,
            // GothicBBB-Medium-RKSJ-H (Gothic BBB)
            L"\x4e2d\x30b4\x30b7\x30c3\x30af\x0042\x0042\x0042",
    IDS_MS_GOTHIC_VERT,
            // GothicBBB-Medium-RKSJ-V (@Gothic BBB)
            L"\x0040\x4e2d\x30b4\x30b7\x30c3\x30af\x0042\x0042\x0042",
    IDS_MS_MINCHO,
            // Ryumin-Light-RKSJ-H (Ryumin L-KL)
            L"\x30ea\x30e5\x30a6\x30df\x30f3\x004c\x002d\x004b\x004c",
    IDS_MS_MINCHO_VERT,
            // Ryumin-Light-RKSJ-V (@ Ryumin L-KL)
            L"\x0040\x30ea\x30e5\x30a6\x30df\x30f3\x004c\x002d\x004b\x004c",
    IDS_MS_GOTHIC,
            // TypeBankG-B-xxxx-H (TypeBank Gothic B)
            L"\xff80\xff72\xff8c\xff9f\xff8a\xff9e\xff9d\xff78"
            L"\xff7a\xff9e\xff7c\xff6f\xff78\x0042",
    IDS_MS_GOTHIC_VERT,
            // TypeBankG-B-xxxx-V (@ TypeBank Gothic B)
            L"\x0040\xff80\xff72\xff8c\xff9f\xff8a\xff9e\xff9d\xff78"
            L"\xff7a\xff9e\xff7c\xff6f\xff78\x0042",
    IDS_MS_MINCHO,
            // TypeBankM-M-xxxx-H (TypeBank Mincho M)
            L"\xff80\xff72\xff8c\xff9f\xff8a\xff9e\xff9d\xff78"
            L"\x660e\x671d\x004d",
    IDS_MS_MINCHO_VERT,
            // TypeBankM-M-xxxx-V (@ TypeBank Mincho M)
            L"\x0040\xff80\xff72\xff8c\xff9f\xff8a\xff9e\xff9d\xff78"
            L"\x660e\x671d\x004d",
    IDS_MS_GOTHIC,
            // HeiseiKakuGothicW5-Medium-xxxx-H (Heisei Kaku Gothic Tai W5)
            L"\x5e73\x6210\x89d2\x30b4\x30b7\x30c3\x30af\x4f53\x0057\x0035",
    IDS_MS_GOTHIC_VERT,
            // HeiseiKakuGothicW5-Medium-xxxx-V (@ Heisei Kaku Gothic Tai W5)
            L"\x0040\x5e73\x6210\x89d2\x30b4\x30b7\x30c3\x30af\x4f53\x0057\x0035",
    IDS_MS_MINCHO,
            // HeiseiMinchoW3-Light-xxxx-H (Heisei Mincho Tai W3)
            L"\x5e73\x6210\x660e\x671d\x4f53\x0057\x0033",
    IDS_MS_MINCHO_VERT,
            // HeiseiMinchoW3-Light-xxxx-V (@ Heisei Mincho Tai W3)
            L"\x0040\x5e73\x6210\x660e\x671d\x4f53\x0057\x0033",
    IDS_MS_GOTHIC,
            // RicohGothicM-xxxx-H (Gothic M)
            L"\x30b4\x30b7\x30c3\x30af\x004d",
    IDS_MS_GOTHIC_VERT,
            // RicohGothicM-xxxx-V (@ Gothic M)
            L"\x0040\x30b4\x30b7\x30c3\x30af\x004d",
    IDS_MS_MINCHO,
            // RicohMinchoL-xxxx-H (Mincho L)
            L"\x660e\x671d\x004c",
    IDS_MS_MINCHO_VERT,
            // RicohMinchoL-xxxx-V (@ Mincho L)
            L"\x0040\x660e\x671d\x004c",
    // FE fonts (CHT)
    IDS_MS_MINGLI,
            // Sung-Light-BIG5
            L"\x7d30\x5b8b\x9ad4",
...
WORD
GetOutGlyPreferOption(
    HANDLE          hprinter )
{
    PPRINTERDATA    pPrinterData;
	WORD            ret = OGPO_DEFAULT;
    ...
    if (pPrinterData != NULL) {
        ...
        // version, assume it is NormalMode ( JIS78 disable )

        if (pPrinterData->wDriverVersion < DRIVER_VERSION) 
            return ret;
                
        ret = pPrinterData->wCurOutGlyPreferOpt; 
    }
...
#if defined(DBCS)
        // We take care of this also in kernel mode.

        if (GetACP() != 1252 && !IsDBCSCodePage(GetACP()))
            pPrinterData->dwFlags |= PSDEV_IGNORE_DEVFONT;
