...
int
__cdecl
iCompareDevFontOptParam(
    const void *pOptParam1,
    const void *pOptParam2
    )
{
    return wcscmp((PWSTR)((POPTPARAM)pOptParam1)->pData,
                    (PWSTR)((POPTPARAM)pOptParam2)->pData);
}
#endif // DBCS
...
#if defined(DBCS) // FE font
    // Using xlated naems, the list is not necessarilty sorted.

    if (pOptType->Count > 1) {
        qsort((pOptType->pOptParam + 1), (pOptType->Count - 1),
                sizeof(*pOptType->pOptParam), iCompareDevFontOptParam);
    }
#endif // DBCS
...

{
    ...
    if (pTTSubstTable == NULL) {
        ...
        if (pTTSubstTable != NULL) {
            ...
            // Assume everay optitem has the same device font options
            // (all available device fonts on this device)

            ASSERT(IsFontSubstItem(pOptItem->UserData));

            for (pwstr = pTTSubstTable; *pwstr != L'\0';) {

                pDevFontName = pwstr + wcslen(pwstr) + 1;
                ...
                DBGMSG1(DBG_LEVEL_VERBOSE,
                    "TrueType font \"%ws\"\n", pDevFontName);
                DBGMSG1(DBG_LEVEL_VERBOSE,
                    "mapped to \"%ws\"\n", bFound ? pOptParam[index].pData : L"(N/A)");

                if (!bFound) {

                    // Invalidate this entry if the device font is not
                    // available on the physical device (this is to allow
                    // N to 1 mapping in the default mapping table)

                    *pwstr = L'!';
                }

                pwstr = pDevFontName + wcslen(pDevFontName) + 1;
            }
        }
    }
#else // DBCS
