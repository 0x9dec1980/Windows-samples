...
#ifdef DBCS  // Font Prefer Option
BOOL PackItemOutGlyPreferOptions(PPACKINFO, PPRINTERDATA); 
BOOL bContainJPNFonts(HPPD);
#endif
...
#ifdef DBCS  // Font Prefer Option
		PackItemOutGlyPreferOptions(pPackInfo, pPrinterData) &&
#endif
...
#ifdef DBCS  // Output Glyph Prefer Option   By Weibz, Aug 29, 1996
#define  OUTGLYPH_PREFER_OPT_MAX   8
#define  IDS_OGPO_NULL     0xffff

WORD    OutGlyph_Prefer_Option[OUTGLYPH_PREFER_OPT_MAX] = {
	             IDS_OGPO_DEFAULT,  
                 IDS_OGPO_JIS78,
                 IDS_OGPO_NULL };

BOOL
PackItemOutGlyPreferOptions(
    PPACKINFO pPackInfo,
    PPRINTERDATA pPrinterData
    )

{
    ...
    if ((pOptItem = pPackInfo->pOptItem) != NULL) {
       ...
       select = pPrinterData->wCurOutGlyPreferOpt;

       FILLOPTITEM(pPackInfo->pOptItem,
            pPackInfo->pOptType,
            IDS_OUTGLY_PREFER_OPTION,
            select,
            TVITEM_LEVEL1,
            DMPUB_NONE,
            OUTGLY_PREFER_OPTION_ITEM,
            HELP_INDEX_TTTODEV);

	   pUiData = pPackInfo->pUiData;
	   hppd = pUiData->hppd;
	   if ( bContainJPNFonts( hppd ) == TRUE ) {
           pOutGlyOpt = OutGlyph_Prefer_Option;
	       cOptParam = 0;
	       while ( *pOutGlyOpt != IDS_OGPO_NULL ) {
		       cOptParam ++;
		       pOutGlyOpt ++;
           }
       } else 
		   cOptParam = 1; // at this time, Only JPN device font can set JIS78

	   pOptParam = FillOutOptType(
		              pPackInfo->pOptType, TVOT_LISTBOX, cOptParam, 
	   ...
	   while ( cOptParam -- ) {
		   ...
		   pOptParam->pData = (PWSTR) *pOutGlyOpt ;
		   pOutGlyOpt++;
		   pOptParam++;
	   }	               
       pPackInfo->pOptItem++;
...

{
    for ( ; cOptItem > 0; cOptItem--, pOptItem++) {
        ...
        case OUTGLY_PREFER_OPTION_ITEM:

            pPrinterData->wCurOutGlyPreferOpt = (WORD)(pOptItem->Sel);
            pPrinterData->dwFlags &= ~PSDEV_IGNORE_DEVFONT;
            break;
