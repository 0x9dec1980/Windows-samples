...

{
        ...
        OpenPrinter(pPrinterName, &hPrinter, NULL))
    {
        if (pDriverInfo2 = MyGetPrinterDriver(hPrinter, 2)) {
            ...
            if (hppd = PpdCreate(pDriverInfo2->pDataFile, ghInstance,
				             wCurOutGlyPreferOpt)) {
#else // DBCS
...

{
    ...
    for ( ; cOptItem > 0; cOptItem--, pOptItem++) {
        ...
        case COLOR_ITEM:
            pdm->dmPublic.dmColor =
                (pOptItem->Sel == 1) ? DMCOLOR_COLOR : DMCOLOR_MONOCHROME;
            break;
...

{
    ...
    case CPSUICB_REASON_ECB_CHANGED:
        ...
        if (UnpackDocumentPropertiesItems(
                pUiData, &pUiData->devmode, pCurItem, 1) != OPTION_INDEX_NONE)
        {
            if (pCurItem->pOptType->Type == TVOT_LISTBOX &&
                CheckConstraintsDlg(pUiData, pCurItem, 1, FALSE) == CONFLICT_CANCEL)
            {
                ...
                DocPropShowConstraints(pUiData,
                    pCallbackParam->pOptItem, pCallbackParam->cOptItem);
...

{
    ...
    return (! pPackInfo->pUiData->hppd->bColorDevice) ||
